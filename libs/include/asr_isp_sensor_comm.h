 ...
 * All Rights Reserved.
 */
#ifndef __ASR_CAM_ISP_SENSOR_H__
#define __ASR_CAM_ISP_SENSOR_H__
...
typedef struct asrISP_SENSOR_ATTR_S {
    uint32_t u32SensorId;
} ISP_SENSOR_ATTR_S;

typedef struct asrISP_FIRMWARE_VERSION_S {
    int revisionNumber;
} ISP_FIRMWARE_VERSION_S;

typedef struct asrISP_DEFAULT_SETTING {
...
typedef struct asrAE_LIB_DEFAULT_SETTING {
    const _isp_ae_attr_t *pstAeAttr;
} AE_LIB_DEFAULT_SETTING_S;

typedef struct asrAWB_LIB_DEFAULT_SETTING {
    const _isp_awb_attr_t *pstAwbAttr;
} AWB_LIB_DEFAULT_SETTING_S;

typedef struct asrISP_SENSOR_BLACK_LEVEL_S {
    bool bUpdate;
    uint32_t bitDepth;
    uint32_t sensorBlackLevel[4];
} ISP_SENSOR_BLACK_LEVEL_S;

typedef struct asrISP_SENSOR_IMAGE_MODE_S {
    uint16_t u16Width;
    uint16_t u16Height;
    float u16Fps;
    uint8_t u8SnsMode;
...
typedef struct asrISP_SENSOR_DEFAULT_S {
    const ISP_DEFAULT_SETTING_S *pstIspDefaultSetting;
} ISP_SENSOR_DEFAULT_S;

typedef struct asrISP_SENSOR_VTS_INFO_S {
    int snsLineTime; /*unit ns */
    int snsVts;
    float snsFps;
} ISP_SENSOR_VTS_INFO_S;

typedef struct asrISP_SENSOR_AE_DEFAULT_S {
...
typedef struct asrISP_SENSOR_COMMBUS_S {
    int8_t s8I2cDev;  // i2c devno, < 0 means invalid
} ISP_SENSOR_COMMBUS_S;

typedef struct asrISP_SENSOR_I2C_DATA_S {
...
typedef struct asrISP_SENSOR_REG_INFO_S {
...
typedef struct asrISP_SENSOR_FUNC_S {
...
typedef struct asrISP_SENSOR_AE_FUNC_S {
    /* Notice:
    ...
    /*
     * function:
     *  get aelib setting according to pipelineID, just for asr aelib temporarily.
     * Notice:
     *   In hdr mode, channel 0 means long exposure while channel 1 indicates short exposure.
     * return:
     *  success:0; failed:-1.
...
typedef struct asrISP_SENSOR_AWB_FUNC_S {
    ...
    /*
     * function:
     *  get awblib setting according to pipelineID, just for asr awblib temporarily.
     * Notice:
     *   In hdr mode, channel 0 means long exposure while channel 1 indicates short exposure.
     * return:
     *  success:0; failed:-1.
...
typedef struct asrISP_SENSOR_REGISTER_S {
...
typedef struct asrISP_AF_MOTOR_REGISTER_S {
