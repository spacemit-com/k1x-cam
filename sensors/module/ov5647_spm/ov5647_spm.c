...
struct regval_tab ov5647_spm_vendor_id[] = {
    {0x300A, 0x56},
    {0x300B, 0x47},
};

static SENSOR_MODULE_SETTING_S ov5647_spm_setting = {
    .ispPreviewSetting =
        {
#include "ov5647_spm_rear_primary_isp_setting.h"
        },
    .ispVideoSetting =
        {
#include "ov5647_spm_rear_primary_isp_setting_video.h"
        },
    .cppPreviewSetting =
        {
#include "ov5647_spm_rear_primary_cpp_preview_setting.h"
        },
    .cppVideoSetting =
        {
#include "ov5647_spm_rear_primary_cpp_video_setting.h"
        },
    .cppSnapshotSetting =
        {
#include "ov5647_spm_rear_primary_cpp_snapshot_setting.h"
        },
    .cppNightshotSetting =
        {
#include "ov5647_spm_rear_primary_cpp_nightshot_setting.h"
        },
    .nightshotSetting =
        {
#include "ov5647_spm_rear_primary_nightshot_setting.h"
        },
};

const CapabilityElement ov5647BackCapCfg[] = {
...
/********************************************************************/
static int ov5647_spm_get_sensor_cap_size(int32_t* capArraySize)
{
    SENSORS_CHECK_PARA_POINTER(capArraySize);

    *capArraySize = OV5647_SPM_WORK_MODE_SIZE;
    return 0;
}

static int ov5647_spm_get_sensor_capbility(int32_t capArraySize, SENSOR_CAPABILITY_S* sensor_capability)
{
    int i;

    SENSORS_CHECK_PARA_POINTER(sensor_capability);
    SENSORS_CHECK_PARA_POINTER(sensor_capability->snr_config);
    if (capArraySize != OV5647_SPM_WORK_MODE_SIZE) {
...
static int ov5647_spm_get_sensor_work_info(int32_t work_mode, SENSOR_WORK_INFO_S* snr_info)
{
    SENSORS_CHECK_PARA_POINTER(snr_info);

    // snr_info->i2c_addr = module_i2c_addr;
    // snr_info->id_table = ov5647_spm_vendor_id;
    ...
    snr_info->pstIspDefaultSettings[CAM_ISP_SCENE_PREVIEW] = &ov5647_spm_setting.ispPreviewSetting;
    snr_info->pstIspDefaultSettings[CAM_ISP_SCENE_VIDEO] = &ov5647_spm_setting.ispVideoSetting;

    return 0;
}

static int ov5647_spm_get_device_info(int devId, CapDeviceCapInfo* dev_info)
{
    switch (devId) {
        case 0:  // back
            dev_info->deviceConfig = ov5647BackCapCfg;
            dev_info->deviceConfigCnt = sizeof(ov5647BackCapCfg) / sizeof(ov5647BackCapCfg[0]);
            break;
        default:
...
static int ov5647_spm_get_sensor_vendor_id(SENSOR_VENDOR_ID_S* vendor_id)
{
    SENSORS_CHECK_PARA_POINTER(vendor_id);

    // vendor_id->i2c_addr = module_i2c_addr;
    vendor_id->id_table = ov5647_spm_vendor_id;
    vendor_id->id_table_size = ARRAY_SIZE(ov5647_spm_vendor_id);

    return 0;
}

static int ov5647_spm_get_sensor_i2c_addr(uint8_t* i2c_addr)
{
...
MODULE_OBJ_S ov5647_spm_Obj = {
    .name = "ov5647_spm",
    .pfnGetsnrCapSize = ov5647_spm_get_sensor_cap_size,
    .pfnGetsnrCapbility = ov5647_spm_get_sensor_capbility,
    .pfnGetSnrWorkInfo = ov5647_spm_get_sensor_work_info,
    .pfnGetDevInfo = ov5647_spm_get_device_info,
    .pfnGetSnrVendorId = ov5647_spm_get_sensor_vendor_id,
    .pfnGetSnrI2cAddr = ov5647_spm_get_sensor_i2c_addr,
};
