...
static const uint8_t module_i2c_addr = 0x32; /* I2C Address 7-bit*/
...
static int sc031_spm_get_sensor_cap_size(int32_t* capArraySize)
{
    ...
    *capArraySize = SC031_SPM_WORK_MODE_SIZE;
    return 0;
}
...
static int sc031_spm_get_sensor_capbility(int32_t capArraySize, SENSOR_CAPABILITY_S* sensor_capability)
{
    ...
    for (i = 0; i < SC031_SPM_WORK_MODE_SIZE; i++) {
        switch (i) {
            case SC031_SPM_640x480_10bit_LINEAR_30_1LANE: {
                ...
                sensor_capability->snr_config[i].work_mode = SC031_SPM_640x480_10bit_LINEAR_30_1LANE;
                sensor_capability->snr_config[i].setting = &sc031_spm_setting;
            } break;
            case SC031_SPM_640x480_10bit_LINEAR_50_1LANE: {
                sensor_capability->snr_config[i].width = 640;
                sensor_capability->snr_config[i].height = 480;
                sensor_capability->snr_config[i].bitDepth = 10;
                sensor_capability->snr_config[i].maxFps = 50;
                sensor_capability->snr_config[i].minFps = 45;
                sensor_capability->snr_config[i].image_mode = SENSOR_LINEAR_MODE;
                sensor_capability->snr_config[i].lane_num = 1;
                sensor_capability->snr_config[i].pattern = ISP_BAYER_PATTERN_MONO;//ISP_BAYER_PATTERN_BGGR;
                sensor_capability->snr_config[i].supportPDAF = 0;
                sensor_capability->snr_config[i].work_mode = SC031_SPM_640x480_10bit_LINEAR_50_1LANE;
                sensor_capability->snr_config[i].setting = &sc031_spm_setting;
            } break;
            default: {
...
static int sc031_spm_get_sensor_work_info(int32_t work_mode, SENSOR_WORK_INFO_S* snr_info)
{
    ...
    switch (work_mode) {
        ...
        case SC031_SPM_640x480_10bit_LINEAR_50_1LANE: {
            snr_info->linetime = 12202;  //ns, Tline = hts/pclk
            snr_info->vts = 1639;
            snr_info->f32maxFps = 50;
            snr_info->exp_time[0] = 1639 * snr_info->linetime / 1000;
            snr_info->again[0] = 1 * 0x100;   // Q8 format
            snr_info->dgain[0] = 1 * 0x1000;  // Q12 format
            snr_info->image_mode = SENSOR_LINEAR_MODE;
            snr_info->setting_table = sc031_spm_640x480_10bit_50fps_tab;
            snr_info->setting_table_size = ARRAY_SIZE(sc031_spm_640x480_10bit_50fps_tab);
            snr_info->mipi_clock = 720;  // Mhz
        } break;
        default: {
