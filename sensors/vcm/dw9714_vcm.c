...
static int dw9714_vcm_move(void* vcmHandle, int position);
...
/*******************************************************************/
static int dw9714_write_register(void* handle, uint16_t regAddr, uint16_t value)
{
...
#endif
    // printf("dw9714_write_register regAddr = %x value %x\n",regAddr, value);

    return ret;
}

static int dw9714_read_register(void* handle, uint16_t regAddr, uint16_t* value)
{
    VCM_CONTEXT_S* vcm_context = NULL;
    int ret = 0;
...
#endif
    // printf(" dw9714_read_register regAddr %d  value  %d\n", data.reg, data.val);
    return ret;
}
...
#define dw9714_DATA_SHIFT 0x4
static int dw9714_write_burst_register(void* handle, struct regval_tab* reg_table, int reg_table_num)
{
    VCM_CONTEXT_S* vcm_context = NULL;
    int ret = 0;
...
#else
    {
        ...
        for (i = 0; i < reg_table_num; i++) {
            dw9714_write_register(handle, reg_table[i].reg, reg_table[i].val);
        }
    }
#endif
...
static int dw9714_vcm_stream_on(void* handle)
{
    VCM_CONTEXT_S* vcm_context = NULL;
    int ret = 0;
...
#else
    ret = dw9714_write_burst_register(handle, stream_on_regs, ARRAY_SIZE(stream_on_regs));
#endif
...
static int dw9714_vcm_init(void** pHandle, int i2c_bus, int i2c_addr)
{
    VCM_CONTEXT_S* vcm_context = NULL;
    ...
    if (i2c_bus != -1)
        ...
        vcm_context->twsi_no = 1;
    if (i2c_addr != -1)
        vcm_context->i2c_addr = i2c_addr;
    else
    ...
    dw9714_vcm_stream_on(vcm_context);
    *pHandle = vcm_context;

    return 0;
}

static int dw9714_vcm_deinit(void* handle)
{
    VCM_CONTEXT_S* vcm_context = NULL;
    ...
    for (int i = ((vcm_context->current_position) / 50); i > 0; i--) {
        dw9714_vcm_move(vcm_context, i * 50);
        usleep(2000);
    }
    vcm_context->magic = 0;
    free(vcm_context);
...
static int dw9714_vcm_move(void* vcmHandle, int position)
{
    ...
    dw9714_write_register(vcmHandle, val_buf[0], val_buf[1]);
    // printf("dw9714 vcm move to position %d val 0x%x, 0x%x\n", position, val_buf[0], val_buf[1]);

    return ret;
}

static int dw9714_vcm_get_ops(void* handle, ISP_AF_MOTOR_REGISTER_S* pVcmFuncOps)
{
    VCM_CONTEXT_S* vcm_context = NULL;
    ...
    pVcmFuncOps->pfn_af_motor_move = dw9714_vcm_move;

    return 0;
}

VCM_OBJ_S dw9714VcmObj = {
    .name = VCM_NAME,
    .pfnInit = dw9714_vcm_init,
    .pfnDeinit = dw9714_vcm_deinit,
    .pfnGetVcmOps = dw9714_vcm_get_ops,
};
