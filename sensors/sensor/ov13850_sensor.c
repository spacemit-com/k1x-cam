...
#define SENSOR_MAGIC 0x13850
#define OV13850_NAME "ov13850"
static const unsigned int ov13850_reg_addr_byte = I2C_16BIT; /*byte width of the sensor register address*/
static const unsigned int ov13850_reg_data_byte = I2C_8BIT;  /*byte width of sensor register data*/
...
#define OV13850_VTS_LINES_MAX  (0x7fff)
#define OV13850_EXPO_LINES_MIN (0x0004)

#define OV13850_VTS_ADDR_H (0x380E)
#define OV13850_VTS_ADDR_L (0x380F)
...
#define OV13850_GROUP_ACCESS (0x3208)

/*******************************************************************/
static int ov13850_write_register(void* handle, uint16_t regAddr, uint16_t value)
{
    ...
    reg_data.reg_len = ov13850_reg_addr_byte;
    reg_data.val_len = ov13850_reg_data_byte;
...
static int ov13850_read_register(void* handle, uint16_t regAddr, uint16_t* value)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    struct cam_i2c_data reg_data;
    int ret = 0;
...
static int ov13850_write_burst_register(void* handle, struct regval_tab* reg_table, int reg_table_num)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    reg_table_data.reg_len = ov13850_reg_addr_byte;
    reg_table_data.val_len = ov13850_reg_data_byte;
...
#else
    {
        ...
        for (i = 0; i < reg_table_num; i++) {
            ov13850_write_register(handle, reg_table[i].reg, reg_table[i].val);
        }
    }
#endif
    return ret;
...
#if 0
static int ov13850_read_burst_register(void* handle, struct regval_tab* reg_table, int reg_table_num)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    struct cam_burst_i2c_data reg_table_data;
    int ret = 0;
...
/*******************************************************************/
/*isp sensor function*/
static int ov13850_sensor_write_reg(void* snsHandle, uint32_t regAddr, uint32_t value)
{
    int ret = 0;
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    ret = ov13850_write_register(snsHandle, regAddr, value);
    pthread_mutex_unlock(&sensor_context->apiLock);
    return ret;
}

static int ov13850_sensor_group_reg_start(void* snsHandle)
{
    int ret = 0;
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    pthread_mutex_lock(&sensor_context->apiLock);
    ov13850_write_register(snsHandle, OV13850_GROUP_ACCESS, 0);
    pthread_mutex_unlock(&sensor_context->apiLock);
    return ret;
}

static int ov13850_sensor_group_reg_done(void* snsHandle)
{
    int ret = 0;
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    pthread_mutex_lock(&sensor_context->apiLock);
    ov13850_write_register(snsHandle, OV13850_GROUP_ACCESS, 0x10);
    // ov13850_write_register(snsHandle, OV13850_GROUP_ACCESS, 0xe0);
    pthread_mutex_unlock(&sensor_context->apiLock);
    return ret;
}

static int ov13850_sensor_get_isp_default(void* snsHandle, uint32_t u32ChanelId, uint32_t camScene,
...
static int ov13850_sensor_get_isp_black_level(void* snsHandle, uint32_t u32ChanelId,
...
static int ov13850_sensor_get_reg_info(void* snsHandle, ISP_SENSOR_REGS_INFO_S* pstSensorRegsInfo)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    uint32_t i = 0;
    ...
    if (false == sensor_context->syncInit) {
        ...
        for (i = 0; i < sensor_context->sensorRegs[0].u32RegNum; i++) {
            ...
            sensor_context->sensorRegs[0].astI2cData[i].u32AddrWidth = ov13850_reg_addr_byte;
            sensor_context->sensorRegs[0].astI2cData[i].u32DataWidth = ov13850_reg_data_byte;
        }


        sensor_context->sensorRegs[0].astI2cData[0].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[0].u32RegAddr = OV13850_EXPO_L;  // exposure time
        sensor_context->sensorRegs[0].astI2cData[1].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[1].u32RegAddr = OV13850_EXPO_M;  // exposure time
        sensor_context->sensorRegs[0].astI2cData[2].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[2].u32RegAddr = OV13850_EXPO_H;  // exposure time
        sensor_context->sensorRegs[0].astI2cData[3].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[3].u32RegAddr = OV13850_AGAIN_L;  // analog gain
        sensor_context->sensorRegs[0].astI2cData[4].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[4].u32RegAddr = OV13850_AGAIN_H;  // analog gain
        // sensor_context->sensorRegs[0].astI2cData[5].u8DelayFrmNum = 2;
        // sensor_context->sensorRegs[0].astI2cData[5].u32RegAddr = OV13850_DGAIN_L;  // digital gain
        // sensor_context->sensorRegs[0].astI2cData[6].u8DelayFrmNum = 2;
        // sensor_context->sensorRegs[0].astI2cData[6].u32RegAddr = OV13850_DGAIN_M;  // digital gain
        // sensor_context->sensorRegs[0].astI2cData[7].u8DelayFrmNum = 2;
        // sensor_context->sensorRegs[0].astI2cData[7].u32RegAddr = OV13850_DGAIN_H;  // digital gain
        sensor_context->sensorRegs[0].astI2cData[5].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[5].u32RegAddr = OV13850_VTS_ADDR_L;  // VTS
        sensor_context->sensorRegs[0].astI2cData[6].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[6].u32RegAddr = OV13850_VTS_ADDR_H;  // VTS
...
static int ov13850_sensor_dump_info(void* snsHandle)
{
    int ret = 0;
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    ov13850_read_register(snsHandle, OV13850_VTS_ADDR_H, &reg_val_h);
    ov13850_read_register(snsHandle, OV13850_VTS_ADDR_L, &reg_val_l);
    vts = ((reg_val_h & 0x7f) << 8) | reg_val_l;
    ov13850_read_register(snsHandle, OV13850_EXPO_H, &reg_val_h);
    ov13850_read_register(snsHandle, OV13850_EXPO_M, &reg_val_m);
    ov13850_read_register(snsHandle, OV13850_EXPO_L, &reg_val_l);
    ...
    pthread_mutex_lock(&sensor_context->apiLock);
    CLOG_INFO("ov13850 regs(vts=%d,exptime=%d,again=0x%x,dain =0x%x),struct(initVTS=%d,initFps=%f,vts=%d,expline=%d)",
...
/*ae function*/
static int ov13850_sensor_get_ae_default(void* snsHandle, uint32_t u32ChanelId, ISP_SENSOR_AE_DEFAULT_S* pstSensorAeDft)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    uint32_t exp_time = 0;
    uint32_t again = 0, dgain = 0;
    ...
    /* uint : us */
    // pstSensorAeDft->maxExpTime = (pstSensorState->initVTS - OV13850_VTS_ADJUST) * sensor_context->lineTime / 1000;
    // pstSensorAeDft->minExpTime = OV13850_EXPO_LINES_MIN * sensor_context->lineTime / 1000;

    pthread_mutex_unlock(&sensor_context->apiLock);
    return 0;
}

static int ov13850_sensor_get_expotime_by_fps(void* snsHandle, float f32Fps)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    pthread_mutex_lock(&sensor_context->apiLock);
    minFps = (sensor_context->minVTS * sensor_context->maxFps) / OV13850_VTS_LINES_MAX;
    ...
    max_expotime = (vts - OV13850_VTS_ADJUST) * sensor_context->lineTime / 1000;  // us
out:
    pthread_mutex_unlock(&sensor_context->apiLock);
    return max_expotime;
}

static int ov13850_sensor_fps_set(void* snsHandle, float f32Fps)
{
    int ret = 0;
    SENSOR_CONTEXT_S* sensor_context = NULL;
    uint32_t lines;
    ...
    if (expLine <= (sensor_context->initVTS - OV13850_VTS_ADJUST)) {
...
static int ov13850_sensor_expotime_update(void* snsHandle, uint32_t u32ChanelId, uint32_t u32ExpoTime,
                                          ISP_SENSOR_VTS_INFO_S* pstSensorVtsInfo)
{
    ...
    expLine = (expLine < OV13850_EXPO_LINES_MIN) ? OV13850_EXPO_LINES_MIN : expLine;
    expLine = (expLine > (OV13850_VTS_LINES_MAX - OV13850_VTS_ADJUST)) ? (OV13850_VTS_LINES_MAX - OV13850_VTS_ADJUST)
                                                                       : expLine;
    sensor_context->hdrIntTime[u32ChanelId] = expLine * sensor_context->lineTime / 1000;

    if (expLine > (sensor_context->initVTS - OV13850_VTS_ADJUST))
        sensor_context->vts[0] = expLine + OV13850_VTS_ADJUST;
    else
        sensor_context->vts[0] = sensor_context->initVTS;
...
static int ov13850_sensor_gain_update(void* snsHandle, uint32_t u32ChanelId, uint32_t* pAgainVal, uint32_t* pDgainVal)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    int ret = 0;
...
static int ov13850_get_aelib_default_settings(void* snsHandle, uint32_t u32ChanelId,
...
/*awb function*/
static int ov13850_sensor_get_awb_default(void* snsHandle, uint32_t u32ChanelId,
...
static int ov13850_get_awblib_default_settings(void* snsHandle, uint32_t u32ChanelId,
...
/*******************************************************************/
static int ov13850_init(void** pHandle, int sns_id, uint8_t sns_addr)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    sensor_context->name = OV13850_NAME;
...
static int ov13850_deinit(void* handle)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    if (sensor_context->stream_on_flag == 1) {
        ov13850_write_burst_register(handle, stream_off_regs, ARRAY_SIZE(stream_off_regs));
...
static int ov13850_global_config(void* handle, SENSOR_WORK_INFO_S* work_info)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    int ret = 0;
    ...
    ret = ov13850_write_burst_register(handle, stream_soft_reset_regs, ARRAY_SIZE(stream_soft_reset_regs));
    if (ret) {
        goto out;
    }
    usleep(5000);
    ret = ov13850_write_burst_register(handle, sensor_context->work_info.setting_table,
    ...
    if(work_info->test_pattern_mode == CC_SENSOR_TEST_PATTERN_COLOR_BARS){
        ret = ov13850_write_burst_register(handle, color_bar_regs, ARRAY_SIZE(color_bar_regs));
    }
...
static int ov13850_set_param(void* handle, const SENSOR_INIT_ATTR_S* init_attr)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
...
static int ov13850_stream_on(void* handle)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    int ret = 0;
    uint32_t i = 0;
    ...
    if (ret)
        return ret;
    for (i = 0; i < sensor_context->sensorRegs[0].u32RegNum; i++) {
        ov13850_write_register(handle, sensor_context->sensorRegs[0].astI2cData[i].u32RegAddr,
                               sensor_context->sensorRegs[0].astI2cData[i].u32Data);
    }
    ret = ov13850_write_burst_register(handle, stream_on_regs, ARRAY_SIZE(stream_on_regs));
...
static int ov13850_stream_off(void* handle)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    int ret = 0;
    ...
    pthread_mutex_lock(&sensor_context->apiLock);
    ret = ov13850_write_burst_register(handle, stream_off_regs, ARRAY_SIZE(stream_off_regs));
...
static int ov13850_get_ops(void* handle, ISP_SENSOR_REGISTER_S* pSensorFuncOps)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    pSensorFuncOps->stSensorFunc.pfn_sensor_write_reg = ov13850_sensor_write_reg;
    pSensorFuncOps->stSensorFunc.pfn_sensor_get_isp_default = ov13850_sensor_get_isp_default;
    pSensorFuncOps->stSensorFunc.pfn_sensor_get_isp_black_level = ov13850_sensor_get_isp_black_level;
    pSensorFuncOps->stSensorFunc.pfn_sensor_get_reg_info = ov13850_sensor_get_reg_info;
    pSensorFuncOps->stSensorFunc.pfn_sensor_dump_info = ov13850_sensor_dump_info;
    pSensorFuncOps->stSensorFunc.pfn_sensor_group_regs_start = ov13850_sensor_group_reg_start;
    pSensorFuncOps->stSensorFunc.pfn_sensor_group_regs_done = ov13850_sensor_group_reg_done;

    pSensorFuncOps->stSensorAeFunc.pfn_sensor_get_ae_default = ov13850_sensor_get_ae_default;
    pSensorFuncOps->stSensorAeFunc.pfn_sensor_fps_set = ov13850_sensor_fps_set;
    pSensorFuncOps->stSensorAeFunc.pfn_sensor_get_expotime_by_fps = ov13850_sensor_get_expotime_by_fps;
    pSensorFuncOps->stSensorAeFunc.pfn_sensor_expotime_update = ov13850_sensor_expotime_update;
    pSensorFuncOps->stSensorAeFunc.pfn_sensor_gain_update = ov13850_sensor_gain_update;
    pSensorFuncOps->stSensorAeFunc.pfn_get_aelib_default_settings = ov13850_get_aelib_default_settings;

    pSensorFuncOps->stSensorAwbFunc.pfn_sensor_get_awb_default = ov13850_sensor_get_awb_default;
    pSensorFuncOps->stSensorAwbFunc.pfn_get_awblib_default_settings = ov13850_get_awblib_default_settings;
    pthread_mutex_unlock(&sensor_context->apiLock);

    return 0;
}

static int ov13850_detect_sensor(void* handle, SENSOR_VENDOR_ID_S* vendor_id)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    int ret = 0;
    ...
    if (ret == -1) {
        ...
        ret = ov13850_read_register(sensor_context, 0x302A, &reg_val);  //OV13850_CHIP_REVISION_REG
        CLOG_INFO("detect sensor%d success, chip version: %x, %d", sensor_context->devId, reg_val, ret);
    }
...
SENSOR_OBJ_S ov13850Obj = {
    .name = OV13850_NAME,
    .pfnInit = ov13850_init,
    .pfnDeinit = ov13850_deinit,
    .pfnGloablConfig = ov13850_global_config,
    .pfnSetParam = ov13850_set_param,
    .pfnStreamOn = ov13850_stream_on,
    .pfnStreamOff = ov13850_stream_off,
    .pfnGetSensorOps = ov13850_get_ops,
    .pfnDetectSns = ov13850_detect_sensor,
    .pfnWriteReg = ov13850_write_register,
    .pfnReadReg = ov13850_read_register,
};
