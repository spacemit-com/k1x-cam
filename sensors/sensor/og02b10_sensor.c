...
static const unsigned int og02b10_reg_addr_byte = I2C_16BIT; /*byte width of the sensor register address*/
static const unsigned int og02b10_reg_data_byte = I2C_8BIT;  /*byte width of sensor register data*/
...
static struct regval_tab test_pattern_regs[] = {
    {0x4320, 0x81},
};

#define OG02B10_VTS_ADJUST     (12) /* vts - max_exposure*/
#define OG02B10_VTS_LINES_MAX  (0xffff)
#define OG02B10_EXPO_LINES_MIN (0x0001)

#define OG02B10_VTS_ADDR_H (0x380E)
#define OG02B10_VTS_ADDR_L (0x380F)
#define OG02B10_EXPO_H     (0x3501)
// #define OG02B10_EXPO_M     (0x3511)
#define OG02B10_EXPO_L     (0x3502)
#define OG02B10_AGAIN_H    (0x3509)
#define OG02B10_AGAIN_L    (0x3508)
#define OG02B10_DGAIN_H    (0x350C)
#define OG02B10_DGAIN_M    (0x350B)
#define OG02B10_DGAIN_L    (0x350A)
#define OG02B10_GROUP_ACCESS (0x3208)

/*******************************************************************/
static int og02b10_write_register(void* handle, uint16_t regAddr, uint16_t value)
{
    ...
    reg_data.reg_len = og02b10_reg_addr_byte;
    reg_data.val_len = og02b10_reg_data_byte;
...
static int og02b10_read_register(void* handle, uint16_t regAddr, uint16_t* value)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    struct cam_i2c_data reg_data;
    int ret = 0;
...
static int og02b10_write_burst_register(void* handle, struct regval_tab* reg_table, int reg_table_num)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    reg_table_data.reg_len = og02b10_reg_addr_byte;
    reg_table_data.val_len = og02b10_reg_data_byte;
...
#else
    {
        ...
        for (i = 0; i < reg_table_num; i++) {
            og02b10_write_register(handle, reg_table[i].reg, reg_table[i].val);
        }
    }
#endif
    return ret;
...
#if 0
static int og02b10_read_burst_register(void* handle, struct regval_tab* reg_table, int reg_table_num)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    struct cam_burst_i2c_data reg_table_data;
    int ret = 0;
...
/*******************************************************************/
/*isp sensor function*/
static int og02b10_sensor_write_reg(void* snsHandle, uint32_t regAddr, uint32_t value)
{
    int ret = 0;
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    ret = og02b10_write_register(snsHandle, regAddr, value);
    pthread_mutex_unlock(&sensor_context->apiLock);
    return ret;
}

static int og02b10_sensor_group_reg_start(void* snsHandle)
{
    int ret = 0;
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    pthread_mutex_lock(&sensor_context->apiLock);
    og02b10_write_register(snsHandle, OG02B10_GROUP_ACCESS, 0);
    pthread_mutex_unlock(&sensor_context->apiLock);
    return ret;
}

static int og02b10_sensor_group_reg_done(void* snsHandle)
{
    int ret = 0;
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    pthread_mutex_lock(&sensor_context->apiLock);
    og02b10_write_register(snsHandle, OG02B10_GROUP_ACCESS, 0x10);
    // og02b10_write_register(snsHandle, OG02B10_GROUP_ACCESS, 0xe0);
    pthread_mutex_unlock(&sensor_context->apiLock);
    return ret;
}

static int og02b10_sensor_get_isp_default(void* snsHandle, uint32_t u32ChanelId, uint32_t camScene,
...
static int og02b10_sensor_get_isp_black_level(void* snsHandle, uint32_t u32ChanelId,
...
static int og02b10_sensor_get_reg_info(void* snsHandle, ISP_SENSOR_REGS_INFO_S* pstSensorRegsInfo)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    uint32_t i = 0;
    ...
    if (false == sensor_context->syncInit) {
        ...
        sensor_context->sensorRegs[0].u32RegNum = 9;
        ...
        for (i = 0; i < sensor_context->sensorRegs[0].u32RegNum; i++) {
            ...
            sensor_context->sensorRegs[0].astI2cData[i].u32AddrWidth = og02b10_reg_addr_byte;
            sensor_context->sensorRegs[0].astI2cData[i].u32DataWidth = og02b10_reg_data_byte;
        }

        sensor_context->sensorRegs[0].astI2cData[0].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[0].u32RegAddr = OG02B10_EXPO_L;  // exposure time
        // sensor_context->sensorRegs[0].astI2cData[1].u8DelayFrmNum = 2;
        // sensor_context->sensorRegs[0].astI2cData[1].u32RegAddr = OG02B10_EXPO_M;  // exposure time
        sensor_context->sensorRegs[0].astI2cData[1].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[1].u32RegAddr = OG02B10_EXPO_H;  // exposure time
        sensor_context->sensorRegs[0].astI2cData[2].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[2].u32RegAddr = OG02B10_AGAIN_L;  // analog gain
        sensor_context->sensorRegs[0].astI2cData[3].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[3].u32RegAddr = OG02B10_AGAIN_H;  // analog gain
        sensor_context->sensorRegs[0].astI2cData[4].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[4].u32RegAddr = OG02B10_DGAIN_L;  // digital gain
        sensor_context->sensorRegs[0].astI2cData[5].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[5].u32RegAddr = OG02B10_DGAIN_M;  // digital gain
        sensor_context->sensorRegs[0].astI2cData[6].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[6].u32RegAddr = OG02B10_DGAIN_H;  // digital gain
        sensor_context->sensorRegs[0].astI2cData[7].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[7].u32RegAddr = OG02B10_VTS_ADDR_L;  // VTS
        sensor_context->sensorRegs[0].astI2cData[8].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[8].u32RegAddr = OG02B10_VTS_ADDR_H;  // VTS
...
static int og02b10_sensor_dump_info(void* snsHandle)
{
    int ret = 0;
// #if 0
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    og02b10_read_register(snsHandle, OG02B10_VTS_ADDR_H, &reg_val_h);
    og02b10_read_register(snsHandle, OG02B10_VTS_ADDR_L, &reg_val_l);
    vts = ((reg_val_h & 0x7f) << 8) | reg_val_l;
    og02b10_read_register(snsHandle, OG02B10_EXPO_H, &reg_val_h);
    // og02b10_read_register(snsHandle, OG02B10_EXPO_M, &reg_val_m);
    og02b10_read_register(snsHandle, OG02B10_EXPO_L, &reg_val_l);
    exp_time = (reg_val_h << 8) | reg_val_l;
    og02b10_read_register(snsHandle, OG02B10_AGAIN_H, &reg_val_h);
    og02b10_read_register(snsHandle, OG02B10_AGAIN_L, &reg_val_l);
    again = ((reg_val_h & 0xf) << 7) | ((reg_val_l & 0x7e) >> 1);
    og02b10_read_register(snsHandle, OG02B10_DGAIN_H, &reg_val_h);
    og02b10_read_register(snsHandle, OG02B10_DGAIN_M, &reg_val_m);
    og02b10_read_register(snsHandle, OG02B10_DGAIN_L, &reg_val_l);
    dgain = ((reg_val_h & 0x3) << 10) | (reg_val_m << 2) | ((reg_val_l & 0xc0) >> 6);

    pthread_mutex_lock(&sensor_context->apiLock);
    CLOG_INFO("og02b10 regs(vts=%d,exptime=%d,again=0x%x,dain =0x%x),struct(initVTS=%d,initFps=%f,vts=%d,expline=%d)",
...
/*ae function*/
static int og02b10_sensor_get_ae_default(void* snsHandle, uint32_t u32ChanelId, ISP_SENSOR_AE_DEFAULT_S* pstSensorAeDft)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    uint32_t exp_time = 0;
    uint32_t again = 0, dgain = 0;
    ...
    /* uint : us */
    // pstSensorAeDft->maxExpTime = (pstSensorState->initVTS - OG02B10_VTS_ADJUST) * sensor_context->lineTime / 1000;
    // pstSensorAeDft->minExpTime = OG02B10_EXPO_LINES_MIN * sensor_context->lineTime / 1000;

    pthread_mutex_unlock(&sensor_context->apiLock);
    return 0;
}

static int og02b10_sensor_get_expotime_by_fps(void* snsHandle, float f32Fps)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    pthread_mutex_lock(&sensor_context->apiLock);
    minFps = (sensor_context->minVTS * sensor_context->maxFps) / OG02B10_VTS_LINES_MAX;
    ...
    max_expotime = (vts - OG02B10_VTS_ADJUST) * sensor_context->lineTime / 1000;  // us
out:
    pthread_mutex_unlock(&sensor_context->apiLock);
    return max_expotime;
}

static int og02b10_sensor_fps_set(void* snsHandle, float f32Fps)
{
    int ret = 0;
    SENSOR_CONTEXT_S* sensor_context = NULL;
    uint32_t lines;
    ...
    if (expLine <= (sensor_context->initVTS - OG02B10_VTS_ADJUST)) {
...
static int og02b10_sensor_expotime_update(void* snsHandle, uint32_t u32ChanelId, uint32_t u32ExpoTime,
                                          ISP_SENSOR_VTS_INFO_S* pstSensorVtsInfo)
{
    ...
    expLine = (expLine < OG02B10_EXPO_LINES_MIN) ? OG02B10_EXPO_LINES_MIN : expLine;
    expLine = (expLine > (OG02B10_VTS_LINES_MAX - OG02B10_VTS_ADJUST)) ? (OG02B10_VTS_LINES_MAX - OG02B10_VTS_ADJUST)
                                                                       : expLine;
    sensor_context->hdrIntTime[u32ChanelId] = expLine * sensor_context->lineTime / 1000;

    if (expLine > (sensor_context->initVTS - OG02B10_VTS_ADJUST))
        sensor_context->vts[0] = expLine + OG02B10_VTS_ADJUST;
    else
        sensor_context->vts[0] = sensor_context->initVTS;
    ...
    pthread_mutex_unlock(&sensor_context->apiLock);
	printf("exp time: %d us, L:%d, initvts:%d\n", u32ExpoTime, expLine, sensor_context->initVTS);

    return 0;
}

static int og02b10_sensor_gain_update(void* snsHandle, uint32_t u32ChanelId, uint32_t* pAgainVal, uint32_t* pDgainVal)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    int ret = 0;
    uint32_t AGain_Reg, DGain_Reg = 0;
    float gain_fval;
    int AGain_Reg_H8, AGain_Reg_L8;
    ...
    pthread_mutex_lock(&sensor_context->apiLock);
    gain_fval = *pAgainVal / 256.0;

    AGain_Reg_H8 = (int)gain_fval;
    AGain_Reg_L8 = (int)((gain_fval - (float)AGain_Reg_H8) * 16.0);
    AGain_Reg = (AGain_Reg_H8 << 8) | (AGain_Reg_L8 << 4);
    ...
    // sensor_context->sensorRegs[0].astI2cData[4].u32Data = (DGain_Reg & 0x0003) << 6;
    // sensor_context->sensorRegs[0].astI2cData[5].u32Data = (DGain_Reg & 0x03fc) >> 2;
    // sensor_context->sensorRegs[0].astI2cData[6].u32Data = ((DGain_Reg & 0x0c00) >> 10);

    *pAgainVal = AGain_Reg << 1;  // Q7 -> Q8
    *pDgainVal = 4096; 
    pthread_mutex_unlock(&sensor_context->apiLock);
printf("again: %x (%x), gain_fval: %f, (%x, %x) (%x)\n", *pAgainVal, tval, gain_fval, AGain_Reg_H8, AGain_Reg_L8, AGain_Reg);

    return ret;
}

static int og02b10_get_aelib_default_settings(void* snsHandle, uint32_t u32ChanelId,
...
/*awb function*/
static int og02b10_sensor_get_awb_default(void* snsHandle, uint32_t u32ChanelId,
...
static int og02b10_get_awblib_default_settings(void* snsHandle, uint32_t u32ChanelId,
...
static int og02b10_power_on(SENSOR_CONTEXT_S *sensor_context)
{
    SENSORS_CHECK_PARA_POINTER(sensor_context);
    CLOG_INFO("%s: power on!", __FUNCTION__);
    sensor_set_power_on(sensor_context->devId, SENSOR_GPIO_PWDN, 1);
    sensor_set_power_on(sensor_context->devId, SENSOR_GPIO_RST, 1);
    sensor_set_mclk_rate(sensor_context->devId, 8000000); // 8M
...
static int og02b10_power_off(SENSOR_CONTEXT_S *sensor_context)
{
    SENSORS_CHECK_PARA_POINTER(sensor_context);
    sensor_set_gpio_enable(sensor_context->devId, SENSOR_GPIO_PWDN, 1);
    sensor_set_gpio_enable(sensor_context->devId, SENSOR_GPIO_RST, 0);
    ...
    sensor_set_power_on(sensor_context->devId, SENSOR_GPIO_RST, 0);
    sensor_set_power_on(sensor_context->devId, SENSOR_GPIO_PWDN, 0);

    return 0;
} 
/*******************************************************************/
static int og02b10_init(void** pHandle, int sns_id, uint8_t sns_addr)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    sensor_context->name = OG02B10_NAME;
    ...
    // og02b10_power_on(sensor_context);
...
static int og02b10_deinit(void* handle)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    if (sensor_context->stream_on_flag == 1) {
        og02b10_write_burst_register(handle, stream_off_regs, ARRAY_SIZE(stream_off_regs));
        sensor_context->stream_on_flag = 0;
    }

    sensor_hw_reset(sensor_context->devId);
    // og02b10_power_off(sensor_context);
...
static int og02b10_global_config(void* handle, SENSOR_WORK_INFO_S* work_info)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    int ret = 0;
    ...
    ret = og02b10_write_burst_register(handle, stream_soft_reset_regs, ARRAY_SIZE(stream_soft_reset_regs));
    if (ret) {
        goto out;
    }
    usleep(5000);
    ret = og02b10_write_burst_register(handle, sensor_context->work_info.setting_table,
    ...
    // work_info->test_pattern_mode = CC_SENSOR_TEST_PATTERN_PN9;
    if(work_info->test_pattern_mode == CC_SENSOR_TEST_PATTERN_COLOR_BARS){
        // ret = og02b10_write_burst_register(handle, color_bar_regs, ARRAY_SIZE(color_bar_regs));
    }
    else if(work_info->test_pattern_mode == CC_SENSOR_TEST_PATTERN_PN9)
    {
        ret = og02b10_write_burst_register(handle, test_pattern_regs, ARRAY_SIZE(test_pattern_regs));
    }
...
static int og02b10_set_param(void* handle, const SENSOR_INIT_ATTR_S* init_attr)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
...
static int og02b10_stream_on(void* handle)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    int ret = 0;
    uint32_t i = 0;
    ...
    if (ret)
        return ret;
    for (i = 0; i < sensor_context->sensorRegs[0].u32RegNum; i++) {
        og02b10_write_register(handle, sensor_context->sensorRegs[0].astI2cData[i].u32RegAddr,
                               sensor_context->sensorRegs[0].astI2cData[i].u32Data);
    }
    ret = og02b10_write_burst_register(handle, stream_on_regs, ARRAY_SIZE(stream_on_regs));
...
static int og02b10_stream_off(void* handle)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    int ret = 0;
    ...
    pthread_mutex_lock(&sensor_context->apiLock);
    ret = og02b10_write_burst_register(handle, stream_off_regs, ARRAY_SIZE(stream_off_regs));
...
static int og02b10_get_ops(void* handle, ISP_SENSOR_REGISTER_S* pSensorFuncOps)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    pSensorFuncOps->stSensorFunc.pfn_sensor_write_reg = og02b10_sensor_write_reg;
    pSensorFuncOps->stSensorFunc.pfn_sensor_get_isp_default = og02b10_sensor_get_isp_default;
    pSensorFuncOps->stSensorFunc.pfn_sensor_get_isp_black_level = og02b10_sensor_get_isp_black_level;
    pSensorFuncOps->stSensorFunc.pfn_sensor_get_reg_info = og02b10_sensor_get_reg_info;
    pSensorFuncOps->stSensorFunc.pfn_sensor_dump_info = og02b10_sensor_dump_info;
    pSensorFuncOps->stSensorFunc.pfn_sensor_group_regs_start = og02b10_sensor_group_reg_start;
    pSensorFuncOps->stSensorFunc.pfn_sensor_group_regs_done = og02b10_sensor_group_reg_done;

    pSensorFuncOps->stSensorAeFunc.pfn_sensor_get_ae_default = og02b10_sensor_get_ae_default;
    pSensorFuncOps->stSensorAeFunc.pfn_sensor_fps_set = og02b10_sensor_fps_set;
    pSensorFuncOps->stSensorAeFunc.pfn_sensor_get_expotime_by_fps = og02b10_sensor_get_expotime_by_fps;
    pSensorFuncOps->stSensorAeFunc.pfn_sensor_expotime_update = og02b10_sensor_expotime_update;
    pSensorFuncOps->stSensorAeFunc.pfn_sensor_gain_update = og02b10_sensor_gain_update;
    pSensorFuncOps->stSensorAeFunc.pfn_get_aelib_default_settings = og02b10_get_aelib_default_settings;

    pSensorFuncOps->stSensorAwbFunc.pfn_sensor_get_awb_default = og02b10_sensor_get_awb_default;
    pSensorFuncOps->stSensorAwbFunc.pfn_get_awblib_default_settings = og02b10_get_awblib_default_settings;
    pthread_mutex_unlock(&sensor_context->apiLock);

    return 0;
}

static int og02b10_detect_sensor(void* handle, SENSOR_VENDOR_ID_S* vendor_id)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    int ret = 0;
...
SENSOR_OBJ_S og02b10Obj = {
    .name = OG02B10_NAME,
    .pfnInit = og02b10_init,
    .pfnDeinit = og02b10_deinit,
    .pfnGloablConfig = og02b10_global_config,
    .pfnSetParam = og02b10_set_param,
    .pfnStreamOn = og02b10_stream_on,
    .pfnStreamOff = og02b10_stream_off,
    .pfnGetSensorOps = og02b10_get_ops,
    .pfnDetectSns = og02b10_detect_sensor,
    .pfnWriteReg = og02b10_write_register,
    .pfnReadReg = og02b10_read_register,
};
