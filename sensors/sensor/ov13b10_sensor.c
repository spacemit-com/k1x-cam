...
#define SENSOR_MAGIC 0x13B105B5
#define OV13B10_NAME "ov13b10"
static const unsigned int ov13b10_reg_addr_byte = I2C_16BIT; /*byte width of the sensor register address*/
static const unsigned int ov13b10_reg_data_byte = I2C_8BIT;  /*byte width of sensor register data*/
...
#define OV13B10_VTS_LINES_MAX  (0x7fff)
#define OV13B10_EXPO_LINES_MIN (0x0004)

#define OV13B10_VTS_ADDR_H (0x380E)
#define OV13B10_VTS_ADDR_L (0x380F)
#define OV13B10_EXPO_H     (0x3500)
#define OV13B10_EXPO_M     (0x3501)
#define OV13B10_EXPO_L     (0x3502)
#define OV13B10_AGAIN_H    (0x3508)
#define OV13B10_AGAIN_L    (0x3509)
#define OV13B10_DGAIN_H    (0x350A)
#define OV13B10_DGAIN_M    (0x350B)
#define OV13B10_DGAIN_L    (0x350C)
#define OV13B10_GROUP_ACCESS (0x3208)

/*******************************************************************/
static int ov13b10_write_register(void* handle, uint16_t regAddr, uint16_t value)
{
    ...
    reg_data.reg_len = ov13b10_reg_addr_byte;
    reg_data.val_len = ov13b10_reg_data_byte;
...
static int ov13b10_read_register(void* handle, uint16_t regAddr, uint16_t* value)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    struct cam_i2c_data reg_data;
    int ret = 0;
...
static int ov13b10_write_burst_register(void* handle, struct regval_tab* reg_table, int reg_table_num)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    reg_table_data.reg_len = ov13b10_reg_addr_byte;
    reg_table_data.val_len = ov13b10_reg_data_byte;
...
#else
    {
        ...
        for (i = 0; i < reg_table_num; i++) {
            ov13b10_write_register(handle, reg_table[i].reg, reg_table[i].val);
        }
    }
#endif
    return ret;
...
#if 0
static int ov13b10_read_burst_register(void* handle, struct regval_tab* reg_table, int reg_table_num)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    struct cam_burst_i2c_data reg_table_data;
    int ret = 0;
...
/*******************************************************************/
/*isp sensor function*/
static int ov13b10_sensor_write_reg(void* snsHandle, uint32_t regAddr, uint32_t value)
{
    int ret = 0;
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    ret = ov13b10_write_register(snsHandle, regAddr, value);
    pthread_mutex_unlock(&sensor_context->apiLock);
    return ret;
}

static int ov13b10_sensor_group_reg_start(void* snsHandle)
{
    int ret = 0;
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    pthread_mutex_lock(&sensor_context->apiLock);
    ov13b10_write_register(snsHandle, OV13B10_GROUP_ACCESS, 0);
    pthread_mutex_unlock(&sensor_context->apiLock);
    return ret;
}

static int ov13b10_sensor_group_reg_done(void* snsHandle)
{
    int ret = 0;
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    pthread_mutex_lock(&sensor_context->apiLock);
    ov13b10_write_register(snsHandle, OV13B10_GROUP_ACCESS, 0x10);
    ov13b10_write_register(snsHandle, OV13B10_GROUP_ACCESS, 0xe0);
    pthread_mutex_unlock(&sensor_context->apiLock);
    return ret;
}

static int ov13b10_sensor_get_isp_default(void* snsHandle, uint32_t u32ChanelId, uint32_t camScene,
...
static int ov13b10_sensor_get_isp_black_level(void* snsHandle, uint32_t u32ChanelId,
...
static int ov13b10_sensor_get_reg_info(void* snsHandle, ISP_SENSOR_REGS_INFO_S* pstSensorRegsInfo)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    uint32_t i = 0;
    ...
    if (false == sensor_context->syncInit) {
        ...
        for (i = 0; i < sensor_context->sensorRegs[0].u32RegNum; i++) {
            ...
            sensor_context->sensorRegs[0].astI2cData[i].u32AddrWidth = ov13b10_reg_addr_byte;
            sensor_context->sensorRegs[0].astI2cData[i].u32DataWidth = ov13b10_reg_data_byte;
        }


        sensor_context->sensorRegs[0].astI2cData[0].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[0].u32RegAddr = OV13B10_EXPO_L;  // exposure time
        sensor_context->sensorRegs[0].astI2cData[1].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[1].u32RegAddr = OV13B10_EXPO_M;  // exposure time
        sensor_context->sensorRegs[0].astI2cData[2].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[2].u32RegAddr = OV13B10_EXPO_H;  // exposure time
        sensor_context->sensorRegs[0].astI2cData[3].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[3].u32RegAddr = OV13B10_AGAIN_L;  // analog gain
        sensor_context->sensorRegs[0].astI2cData[4].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[4].u32RegAddr = OV13B10_AGAIN_H;  // analog gain
        sensor_context->sensorRegs[0].astI2cData[5].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[5].u32RegAddr = OV13B10_DGAIN_L;  // digital gain
        sensor_context->sensorRegs[0].astI2cData[6].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[6].u32RegAddr = OV13B10_DGAIN_M;  // digital gain
        sensor_context->sensorRegs[0].astI2cData[7].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[7].u32RegAddr = OV13B10_DGAIN_H;  // digital gain
        sensor_context->sensorRegs[0].astI2cData[8].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[8].u32RegAddr = OV13B10_VTS_ADDR_L;  // VTS
        sensor_context->sensorRegs[0].astI2cData[9].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[9].u32RegAddr = OV13B10_VTS_ADDR_H;  // VTS
...
static int ov13b10_sensor_dump_info(void* snsHandle)
{
    int ret = 0;
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    ov13b10_read_register(snsHandle, OV13B10_VTS_ADDR_H, &reg_val_h);
    ov13b10_read_register(snsHandle, OV13B10_VTS_ADDR_L, &reg_val_l);
    vts = ((reg_val_h & 0x7f) << 8) | reg_val_l;
    ov13b10_read_register(snsHandle, OV13B10_EXPO_H, &reg_val_h);
    ov13b10_read_register(snsHandle, OV13B10_EXPO_M, &reg_val_m);
    ov13b10_read_register(snsHandle, OV13B10_EXPO_L, &reg_val_l);
    exp_time = (reg_val_h << 16) | (reg_val_m << 8) | reg_val_l;
    ov13b10_read_register(snsHandle, OV13B10_AGAIN_H, &reg_val_h);
    ov13b10_read_register(snsHandle, OV13B10_AGAIN_L, &reg_val_l);
    again = ((reg_val_h & 0xf) << 7) | ((reg_val_l & 0x7e) >> 1);
    ov13b10_read_register(snsHandle, OV13B10_DGAIN_H, &reg_val_h);
    ov13b10_read_register(snsHandle, OV13B10_DGAIN_M, &reg_val_m);
    ov13b10_read_register(snsHandle, OV13B10_DGAIN_L, &reg_val_l);
    dgain = ((reg_val_h & 0x3) << 10) | (reg_val_m << 2) | ((reg_val_l & 0xc0) >> 6);

    pthread_mutex_lock(&sensor_context->apiLock);
    CLOG_INFO("ov13b10 regs(vts=%d,exptime=%d,again=0x%x,dain =0x%x),struct(initVTS=%d,initFps=%f,vts=%d,expline=%d)",
...
/*ae function*/
static int ov13b10_sensor_get_ae_default(void* snsHandle, uint32_t u32ChanelId, ISP_SENSOR_AE_DEFAULT_S* pstSensorAeDft)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    uint32_t exp_time = 0;
    uint32_t again = 0, dgain = 0;
    ...
    /* uint : us */
    // pstSensorAeDft->maxExpTime = (pstSensorState->initVTS - OV13B10_VTS_ADJUST) * sensor_context->lineTime / 1000;
    // pstSensorAeDft->minExpTime = OV13B10_EXPO_LINES_MIN * sensor_context->lineTime / 1000;

    pthread_mutex_unlock(&sensor_context->apiLock);
    return 0;
}

static int ov13b10_sensor_get_expotime_by_fps(void* snsHandle, float f32Fps)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    pthread_mutex_lock(&sensor_context->apiLock);
    minFps = (sensor_context->minVTS * sensor_context->maxFps) / OV13B10_VTS_LINES_MAX;
    ...
    max_expotime = (vts - OV13B10_VTS_ADJUST) * sensor_context->lineTime / 1000;  // us
out:
    pthread_mutex_unlock(&sensor_context->apiLock);
    return max_expotime;
}

static int ov13b10_sensor_fps_set(void* snsHandle, float f32Fps)
{
    int ret = 0;
    SENSOR_CONTEXT_S* sensor_context = NULL;
    uint32_t lines;
    ...
    if (expLine <= (sensor_context->initVTS - OV13B10_VTS_ADJUST)) {
...
static int ov13b10_sensor_expotime_update(void* snsHandle, uint32_t u32ChanelId, uint32_t u32ExpoTime,
                                          ISP_SENSOR_VTS_INFO_S* pstSensorVtsInfo)
{
    ...
    expLine = (expLine < OV13B10_EXPO_LINES_MIN) ? OV13B10_EXPO_LINES_MIN : expLine;
    expLine = (expLine > (OV13B10_VTS_LINES_MAX - OV13B10_VTS_ADJUST)) ? (OV13B10_VTS_LINES_MAX - OV13B10_VTS_ADJUST)
                                                                       : expLine;
    sensor_context->hdrIntTime[u32ChanelId] = expLine * sensor_context->lineTime / 1000;

    if (expLine > (sensor_context->initVTS - OV13B10_VTS_ADJUST))
        sensor_context->vts[0] = expLine + OV13B10_VTS_ADJUST;
    else
        sensor_context->vts[0] = sensor_context->initVTS;
...
static int ov13b10_sensor_gain_update(void* snsHandle, uint32_t u32ChanelId, uint32_t* pAgainVal, uint32_t* pDgainVal)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    int ret = 0;
...
static int ov13b10_get_aelib_default_settings(void* snsHandle, uint32_t u32ChanelId,
...
/*awb function*/
static int ov13b10_sensor_get_awb_default(void* snsHandle, uint32_t u32ChanelId,
...
static int ov13b10_get_awblib_default_settings(void* snsHandle, uint32_t u32ChanelId,
...
/*******************************************************************/
static int ov13b10_init(void** pHandle, int sns_id, uint8_t sns_addr)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    sensor_context->name = OV13B10_NAME;
...
static int ov13b10_deinit(void* handle)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    if (sensor_context->stream_on_flag == 1) {
        ov13b10_write_burst_register(handle, stream_off_regs, ARRAY_SIZE(stream_off_regs));
...
static int ov13b10_global_config(void* handle, SENSOR_WORK_INFO_S* work_info)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    int ret = 0;
    ...
    ret = ov13b10_write_burst_register(handle, stream_soft_reset_regs, ARRAY_SIZE(stream_soft_reset_regs));
    if (ret) {
        goto out;
    }
    usleep(5000);
    ret = ov13b10_write_burst_register(handle, sensor_context->work_info.setting_table,
    ...
    if(work_info->test_pattern_mode == CC_SENSOR_TEST_PATTERN_COLOR_BARS){
        ret = ov13b10_write_burst_register(handle, color_bar_regs, ARRAY_SIZE(color_bar_regs));
    }
...
static int ov13b10_set_param(void* handle, const SENSOR_INIT_ATTR_S* init_attr)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
...
static int ov13b10_stream_on(void* handle)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    int ret = 0;
    uint32_t i = 0;
    ...
    ret = ov13b10_write_burst_register(handle, stream_on_regs, ARRAY_SIZE(stream_on_regs));
...
static int ov13b10_stream_off(void* handle)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    int ret = 0;
    ...
    pthread_mutex_lock(&sensor_context->apiLock);
    ret = ov13b10_write_burst_register(handle, stream_off_regs, ARRAY_SIZE(stream_off_regs));
...
static int ov13b10_get_ops(void* handle, ISP_SENSOR_REGISTER_S* pSensorFuncOps)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    pSensorFuncOps->stSensorFunc.pfn_sensor_write_reg = ov13b10_sensor_write_reg;
    pSensorFuncOps->stSensorFunc.pfn_sensor_get_isp_default = ov13b10_sensor_get_isp_default;
    pSensorFuncOps->stSensorFunc.pfn_sensor_get_isp_black_level = ov13b10_sensor_get_isp_black_level;
    pSensorFuncOps->stSensorFunc.pfn_sensor_get_reg_info = ov13b10_sensor_get_reg_info;
    pSensorFuncOps->stSensorFunc.pfn_sensor_dump_info = ov13b10_sensor_dump_info;
    pSensorFuncOps->stSensorFunc.pfn_sensor_group_regs_start = ov13b10_sensor_group_reg_start;
    pSensorFuncOps->stSensorFunc.pfn_sensor_group_regs_done = ov13b10_sensor_group_reg_done;

    pSensorFuncOps->stSensorAeFunc.pfn_sensor_get_ae_default = ov13b10_sensor_get_ae_default;
    pSensorFuncOps->stSensorAeFunc.pfn_sensor_fps_set = ov13b10_sensor_fps_set;
    pSensorFuncOps->stSensorAeFunc.pfn_sensor_get_expotime_by_fps = ov13b10_sensor_get_expotime_by_fps;
    pSensorFuncOps->stSensorAeFunc.pfn_sensor_expotime_update = ov13b10_sensor_expotime_update;
    pSensorFuncOps->stSensorAeFunc.pfn_sensor_gain_update = ov13b10_sensor_gain_update;
    pSensorFuncOps->stSensorAeFunc.pfn_get_aelib_default_settings = ov13b10_get_aelib_default_settings;

    pSensorFuncOps->stSensorAwbFunc.pfn_sensor_get_awb_default = ov13b10_sensor_get_awb_default;
    pSensorFuncOps->stSensorAwbFunc.pfn_get_awblib_default_settings = ov13b10_get_awblib_default_settings;
    pthread_mutex_unlock(&sensor_context->apiLock);

    return 0;
}

static int ov13b10_detect_sensor(void* handle, SENSOR_VENDOR_ID_S* vendor_id)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    int ret = 0;
...
SENSOR_OBJ_S ov13b10Obj = {
    .name = OV13B10_NAME,
    .pfnInit = ov13b10_init,
    .pfnDeinit = ov13b10_deinit,
    .pfnGloablConfig = ov13b10_global_config,
    .pfnSetParam = ov13b10_set_param,
    .pfnStreamOn = ov13b10_stream_on,
    .pfnStreamOff = ov13b10_stream_off,
    .pfnGetSensorOps = ov13b10_get_ops,
    .pfnDetectSns = ov13b10_detect_sensor,
