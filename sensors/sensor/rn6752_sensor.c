...
#define SENSOR_MAGIC 0x6752
#define RN6752_NAME "rn6752"
#define PAGE_SELECT_REG		0xFF
#define PAGE_ZERO			0x00
#define PAGE_ONE			0x01
#define PAGE_TWO			0x02
static const unsigned int rn6752_reg_addr_byte = I2C_8BIT; /*byte width of the sensor register address*/
...
#define RN6752_VTS_ADJUST     (0) /* vts - max_exposure, RN6752 is video decoder, no VTS adjust needed */
#define RN6752_VTS_LINES_MAX  (0x7fff)
#define RN6752_EXPO_LINES_MIN (0x0001)

// RN6752 is a video decoder, it doesn't have traditional exposure/gain registers
// These are placeholder definitions for compatibility
#define RN6752_VTS_ENABLE_REG (0x00)
#define RN6752_VTS_ENABLE_VALUE	0x00
#define RN6752_VTS_ADDR_H (0x00)
#define RN6752_VTS_ADDR_L (0x00)
#define RN6752_EXPO_H     (0x00)
#define RN6752_EXPO_L     (0x00)
#define RN6752_AGAIN_REG    (0x00)
#define RN6752_PAGE_ADDR    (0xFF)

/*******************************************************************/
static int rn6752_write_register(void* handle, uint16_t regAddr, uint16_t value)
{
    ...
    reg_data.reg_len = rn6752_reg_addr_byte;
    reg_data.val_len = rn6752_reg_data_byte;
...
static int rn6752_read_register(void* handle, uint16_t regAddr, uint16_t* value)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    struct cam_i2c_data reg_data;
    int ret = 0;
...
static int rn6752_write_burst_register(void* handle, struct regval_tab* reg_table, int reg_table_num)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    int ret = 0;
...

    {
        ...
        for (i = 0; i < reg_table_num; i++) {
            ret = rn6752_write_register(handle, reg_table[i].reg, reg_table[i].val);
            if (ret) {
...
/*******************************************************************/
/*isp sensor function*/
static int rn6752_sensor_write_reg(void* snsHandle, uint32_t regAddr, uint32_t value)
{
    int ret = 0;
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    ret = rn6752_write_register(snsHandle, regAddr, value);
    pthread_mutex_unlock(&sensor_context->apiLock);
    return ret;
}

static int rn6752_sensor_group_reg_start(void* snsHandle)
{
    int ret = 0;
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    // RN6752 doesn't support group register access
    return ret;
}

static int rn6752_sensor_group_reg_done(void* snsHandle)
{
    int ret = 0;
    SENSOR_CONTEXT_S* sensor_context = NULL;
...
static int rn6752_sensor_get_isp_default(void* snsHandle, uint32_t u32ChanelId, uint32_t camScene,
...
static int rn6752_sensor_get_isp_black_level(void* snsHandle, uint32_t u32ChanelId,
                                              ISP_SENSOR_BLACK_LEVEL_S* pstBlackLevel)
{
    ...
    if (SENSOR_LINEAR_MODE == sensor_context->work_info.image_mode) {
        for (i = 0; i < 4; i++) {
            pstBlackLevel->sensorBlackLevel[i] = 0; /* 8-bit UYVY format, black level is 0 */
...
static int rn6752_sensor_get_reg_info(void* snsHandle, ISP_SENSOR_REGS_INFO_S* pstSensorRegsInfo)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    uint32_t i = 0;
    ...
    if (false == sensor_context->syncInit) {
        // RN6752 is a video decoder, it doesn't have exposure/gain registers
        // Set minimal register info for compatibility
        sensor_context->sensorRegs[0].u8CfgDelayMax = 2;
        sensor_context->sensorRegs[0].u32RegNum = 0;  // No registers to sync
...
static int rn6752_sensor_dump_info(void* snsHandle)
{
    int ret = 0;
    SENSOR_CONTEXT_S* sensor_context = NULL;
    uint16_t chip_id_h = 0, chip_id_l = 0;

    SENSORS_CHECK_PARA_POINTER(snsHandle);
    sensor_context = (SENSOR_CONTEXT_S*)snsHandle;
    SENSOR_CHECK_HANDLE_IS_ERR(sensor_context);

    rn6752_read_register(snsHandle, 0xFE, &chip_id_h);
    rn6752_read_register(snsHandle, 0xFD, &chip_id_l);

    pthread_mutex_lock(&sensor_context->apiLock);
    CLOG_INFO("rn6752 regs(chip_id_h=0x%02x,chip_id_l=0x%02x),struct(initVTS=%d,initFps=%f,vts=%d)",
        chip_id_h, chip_id_l, sensor_context->initVTS, sensor_context->initFps, sensor_context->vts[0]);
    pthread_mutex_unlock(&sensor_context->apiLock);
...
/*ae function*/
static int rn6752_sensor_get_ae_default(void* snsHandle, uint32_t u32ChanelId, ISP_SENSOR_AE_DEFAULT_S* pstSensorAeDft)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    uint32_t exp_time = 0;
...
static int rn6752_sensor_get_expotime_by_fps(void* snsHandle, float f32Fps)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    pthread_mutex_lock(&sensor_context->apiLock);
    minFps = (sensor_context->minVTS * sensor_context->maxFps) / RN6752_VTS_LINES_MAX;
    ...
    max_expotime = (vts - RN6752_VTS_ADJUST) * sensor_context->lineTime / 1000;  // us
out:
    pthread_mutex_unlock(&sensor_context->apiLock);
    return max_expotime;
}

static int rn6752_sensor_fps_set(void* snsHandle, float f32Fps)
{
    int ret = 0;
    SENSOR_CONTEXT_S* sensor_context = NULL;
    uint32_t lines;
...
static int rn6752_sensor_expotime_update(void* snsHandle, uint32_t u32ChanelId, uint32_t u32ExpoTime,
                                          ISP_SENSOR_VTS_INFO_S* pstSensorVtsInfo)
{
    ...
    expLine = (expLine < RN6752_EXPO_LINES_MIN) ? RN6752_EXPO_LINES_MIN : expLine;
    expLine = (expLine > (RN6752_VTS_LINES_MAX - RN6752_VTS_ADJUST)) ? (RN6752_VTS_LINES_MAX - RN6752_VTS_ADJUST)
                                                                       : expLine;
    sensor_context->hdrIntTime[u32ChanelId] = expLine * sensor_context->lineTime / 1000;

    if (expLine > (sensor_context->initVTS - RN6752_VTS_ADJUST))
        sensor_context->vts[0] = expLine + RN6752_VTS_ADJUST;
    else
        sensor_context->vts[0] = sensor_context->initVTS;
...
static int rn6752_sensor_gain_update(void* snsHandle, uint32_t u32ChanelId, uint32_t* pAgainVal, uint32_t* pDgainVal)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    int ret = 0;
    ...
    pthread_mutex_lock(&sensor_context->apiLock);
    // RN6752 is a video decoder, it doesn't support gain control
    // Keep the values as-is for compatibility
    *pAgainVal = *pAgainVal;
    *pDgainVal = *pDgainVal;
    pthread_mutex_unlock(&sensor_context->apiLock);
...
static int rn6752_get_aelib_default_settings(void* snsHandle, uint32_t u32ChanelId,
...
/*awb function*/
static int rn6752_sensor_get_awb_default(void* snsHandle, uint32_t u32ChanelId,
...
static int rn6752_get_awblib_default_settings(void* snsHandle, uint32_t u32ChanelId,
...
static int rn6752_power_on(SENSOR_CONTEXT_S* sensor_context)
{
    SENSORS_CHECK_PARA_POINTER(sensor_context);

    /* Step 1: Enable DVDD power (1.2V) - Reference: rn6752.c line 832-848 */
    ...
    /* Step 2: Release powerdown GPIO (set to 0) - Reference: rn6752.c line 854-859 */
    sensor_set_gpio_enable(sensor_context->devId, SENSOR_GPIO_PWDN, 0);
    usleep(15000);  /* Wait 10-20ms after powerdown release - Reference: rn6752.c line 861-862 */

    /* Step 3: Reset sequence - assert reset (set to 1) - Reference: rn6752.c line 865-868 */
    sensor_set_gpio_enable(sensor_context->devId, SENSOR_GPIO_RST, 1);
    usleep(15000);  /* Wait 10-20ms after assert reset - Reference: rn6752.c line 868 */

    /* Step 4: Reset sequence - deassert reset (set to 0) - Reference: rn6752.c line 869-872 */
    sensor_set_gpio_enable(sensor_context->devId, SENSOR_GPIO_RST, 0);
    usleep(15000);  /* Wait 10-20ms after deassert reset - Reference: rn6752.c line 871 */

    CLOG_INFO("finish power on");
    return 0;
}
/*******************************************************************/
static int rn6752_init(void** pHandle, SENSOR_CUSTOM_S snr_custom)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    sensor_context->name = RN6752_NAME;
    ...
    rn6752_power_on(sensor_context);
...
static int rn6752_power_off(SENSOR_CONTEXT_S* sensor_context)
{
    SENSORS_CHECK_PARA_POINTER(sensor_context);

    /* Step 1: Assert reset GPIO (set to 1) - Reference: rn6752.c line 892-895 */
    sensor_set_gpio_enable(sensor_context->devId, SENSOR_GPIO_RST, 1);

    /* Step 2: Assert powerdown GPIO (set to 1) - Reference: rn6752.c line 897-900 */
    sensor_set_gpio_enable(sensor_context->devId, SENSOR_GPIO_PWDN, 1);

    /* Step 3: Disable DVDD power - Reference: rn6752.c line 902-905 */
    sensor_set_power_on(sensor_context->devId, SENSOR_REGULATOR_DVDD, 0);

    CLOG_INFO("finish power off");
    return 0;
}
static int rn6752_deinit(void* handle)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    if (sensor_context->stream_on_flag == 1) {
        rn6752_write_burst_register(handle, stream_off_regs, ARRAY_SIZE(stream_off_regs));
        sensor_context->stream_on_flag = 0;
    }

    rn6752_power_off(sensor_context);
...
static int rn6752_global_config(void* handle, SENSOR_WORK_INFO_S* work_info)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    int ret = 0;
    ...
    ret = rn6752_write_burst_register(handle, stream_soft_reset_regs, ARRAY_SIZE(stream_soft_reset_regs));
    if (ret) {
        goto out;
    }
    usleep(5000);
    ret = rn6752_write_burst_register(handle, sensor_context->work_info.setting_table,
    ...
    if(work_info->test_pattern_mode == CC_SENSOR_TEST_PATTERN_COLOR_BARS){
        // ret = rn6752_write_burst_register(handle, color_bar_regs, ARRAY_SIZE(color_bar_regs));
    }
...
static int rn6752_set_param(void* handle, const SENSOR_INIT_ATTR_S* init_attr)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
...
static int rn6752_stream_on(void* handle)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    int ret = 0;
...
static int rn6752_stream_off(void* handle)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    int ret = 0;
    ...
    pthread_mutex_lock(&sensor_context->apiLock);
    ret = rn6752_write_burst_register(handle, stream_off_regs, ARRAY_SIZE(stream_off_regs));
...
static int rn6752_get_ops(void* handle, ISP_SENSOR_REGISTER_S* pSensorFuncOps)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    pSensorFuncOps->stSensorFunc.pfn_sensor_write_reg = rn6752_sensor_write_reg;
    pSensorFuncOps->stSensorFunc.pfn_sensor_get_isp_default = rn6752_sensor_get_isp_default;
    pSensorFuncOps->stSensorFunc.pfn_sensor_get_isp_black_level = rn6752_sensor_get_isp_black_level;
    pSensorFuncOps->stSensorFunc.pfn_sensor_get_reg_info = rn6752_sensor_get_reg_info;
    pSensorFuncOps->stSensorFunc.pfn_sensor_dump_info = rn6752_sensor_dump_info;
    pSensorFuncOps->stSensorFunc.pfn_sensor_group_regs_start = rn6752_sensor_group_reg_start;
    pSensorFuncOps->stSensorFunc.pfn_sensor_group_regs_done = rn6752_sensor_group_reg_done;

    pSensorFuncOps->stSensorAeFunc.pfn_sensor_get_ae_default = rn6752_sensor_get_ae_default;
    pSensorFuncOps->stSensorAeFunc.pfn_sensor_fps_set = rn6752_sensor_fps_set;
    pSensorFuncOps->stSensorAeFunc.pfn_sensor_get_expotime_by_fps = rn6752_sensor_get_expotime_by_fps;
    pSensorFuncOps->stSensorAeFunc.pfn_sensor_expotime_update = rn6752_sensor_expotime_update;
    pSensorFuncOps->stSensorAeFunc.pfn_sensor_gain_update = rn6752_sensor_gain_update;
    pSensorFuncOps->stSensorAeFunc.pfn_get_aelib_default_settings = rn6752_get_aelib_default_settings;

    pSensorFuncOps->stSensorAwbFunc.pfn_sensor_get_awb_default = rn6752_sensor_get_awb_default;
    pSensorFuncOps->stSensorAwbFunc.pfn_get_awblib_default_settings = rn6752_get_awblib_default_settings;
    pthread_mutex_unlock(&sensor_context->apiLock);

    return 0;
}

static int rn6752_detect_sensor(void* handle, SENSOR_VENDOR_ID_S* vendor_id)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    int ret = 0;
    ...
    ret = rn6752_write_register(handle, PAGE_SELECT_REG, PAGE_ZERO);
    if (ret) {
    ...
    reg_table_data.reg_len = rn6752_reg_addr_byte;
    reg_table_data.val_len = rn6752_reg_data_byte;
...
SENSOR_OBJ_S rn6752Obj = {
    .name = RN6752_NAME,
    .pfnInit = rn6752_init,
    .pfnDeinit = rn6752_deinit,
    .pfnGloablConfig = rn6752_global_config,
    .pfnSetParam = rn6752_set_param,
    .pfnStreamOn = rn6752_stream_on,
    .pfnStreamOff = rn6752_stream_off,
    .pfnGetSensorOps = rn6752_get_ops,
    .pfnDetectSns = rn6752_detect_sensor,
    .pfnWriteReg = rn6752_write_register,
    .pfnReadReg = rn6752_read_register,
};
