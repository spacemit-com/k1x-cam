...
#define SENSOR_MAGIC 0x12250 /* "SCSC" */
#define SC520SC_NAME "sc520sc"
...
static const unsigned int sc520sc_reg_addr_byte = I2C_16BIT; /*byte width of the sensor register address*/
static const unsigned int sc520sc_reg_data_byte = I2C_8BIT;  /*byte width of sensor register data*/
...
#define SC520SC_VTS_ADJUST     (5) /* vts - max_exposure*/
#define SC520SC_VTS_LINES_MAX  (0x7fff)
#define SC520SC_EXPO_LINES_MIN (0x0004)

#define SC520SC_VTS_ADDR_H   (0x320e)
#define SC520SC_VTS_ADDR_L   (0x320f)
#define SC520SC_EXPO_H       (0x3e00)
#define SC520SC_EXPO_M       (0x3e01)
#define SC520SC_EXPO_L       (0x3e02)

#define SC520SC_ANA_AGAIN           (0x3e09)
#define SC520SC_DIG_GAIN     (0x3e07)
#define SC520SC_ANA_GAIN     (0x3e09)
#define SC520SC_GROUP_ACCESS (0x3812)
#define SC520SC_GROUP_DELAY (0x3802)
/*******************************************************************/
...
static int sc520sc_write_register(void* handle, uint16_t regAddr, uint16_t value)
{
    ...
    reg_data.reg_len = sc520sc_reg_addr_byte;
    reg_data.val_len = sc520sc_reg_data_byte;
...
static int sc520sc_read_register(void* handle, uint16_t regAddr, uint16_t* value)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    struct cam_i2c_data reg_data;
    int ret = 0;
...
static int sc520sc_write_burst_register(void* handle, struct regval_tab* reg_table, int reg_table_num)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    reg_table_data.reg_len = sc520sc_reg_addr_byte;
    reg_table_data.val_len = sc520sc_reg_data_byte;
    ...
    printf("xhd-test: sc520sc_write_burst_register, ret: %d, num: %d\n", ret, reg_table_num);
#else
    {
        int i;
        for (i = 0; i < reg_table_num; i++) {
            sc520sc_write_register(handle, reg_table[i].reg, reg_table[i].val);
        }
    }
#endif
    return ret;
...
#if 0
static int sc520sc_read_burst_register(void* handle, struct regval_tab* reg_table, int reg_table_num)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    struct cam_burst_i2c_data reg_table_data;
    int ret = 0;
...
/*******************************************************************/
/*isp sensor function*/
static int sc520sc_sensor_write_reg(void* snsHandle, uint32_t regAddr, uint32_t value)
{
    int ret = 0;
    SENSOR_CONTEXT_S* sensor_context = NULL;
    printf("xhd-test: sc520sc_sensor_write_reg\n");
    ...
    ret = sc520sc_write_register(snsHandle, regAddr, value);
    printf("xhd-test: sc520sc_sensor_write_reg, regAddr: 0x%x, value: 0x%x, ret: %d\n", regAddr, value, ret);
    pthread_mutex_unlock(&sensor_context->apiLock);
    return ret;
}

static int sc520sc_sensor_group_reg_start(void* snsHandle)
{
    int ret = 0;
    SENSOR_CONTEXT_S* sensor_context = NULL;
    printf("xhd-test: sc520sc_sensor_group_reg_start\n");
    SENSORS_CHECK_PARA_POINTER(snsHandle);
    sensor_context = (SENSOR_CONTEXT_S*)snsHandle;
    SENSOR_CHECK_HANDLE_IS_ERR(sensor_context);

    pthread_mutex_lock(&sensor_context->apiLock);
    sc520sc_write_register(snsHandle, SC520SC_GROUP_ACCESS, 0);
    // sc520sc_write_register(snsHandle, SC520SC_GROUP_DELAY, 2);
    pthread_mutex_unlock(&sensor_context->apiLock);
    return ret;
}

static int sc520sc_sensor_group_reg_done(void* snsHandle)
{
    int ret = 0;
    SENSOR_CONTEXT_S* sensor_context = NULL;
    printf("xhd-test: sc520sc_sensor_group_reg_done\n");
    SENSORS_CHECK_PARA_POINTER(snsHandle);
    sensor_context = (SENSOR_CONTEXT_S*)snsHandle;
    SENSOR_CHECK_HANDLE_IS_ERR(sensor_context);

    pthread_mutex_lock(&sensor_context->apiLock);
    sc520sc_write_register(snsHandle, SC520SC_GROUP_ACCESS, 0x30);
    // sc520sc_write_register(snsHandle, SC520SC_GROUP_DELAY, 2);
    pthread_mutex_unlock(&sensor_context->apiLock);
    return ret;
}

static int sc520sc_sensor_get_isp_default(void* snsHandle, uint32_t u32ChanelId, uint32_t camScene,
...
static int sc520sc_sensor_get_isp_black_level(void* snsHandle, uint32_t u32ChanelId,
...
static int sc520sc_sensor_get_reg_info(void* snsHandle, ISP_SENSOR_REGS_INFO_S* pstSensorRegsInfo)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    uint32_t i = 0;
    ...
    if (false == sensor_context->syncInit) {
        ...
        for (i = 0; i < sensor_context->sensorRegs[0].u32RegNum; i++) {
            ...
            sensor_context->sensorRegs[0].astI2cData[i].u32AddrWidth = sc520sc_reg_addr_byte;
            sensor_context->sensorRegs[0].astI2cData[i].u32DataWidth = sc520sc_reg_data_byte;
        }

        sensor_context->sensorRegs[0].astI2cData[0].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[0].u32RegAddr = SC520SC_EXPO_L;  // exposure time
        sensor_context->sensorRegs[0].astI2cData[1].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[1].u32RegAddr = SC520SC_EXPO_M;  // exposure time
        sensor_context->sensorRegs[0].astI2cData[2].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[2].u32RegAddr = SC520SC_EXPO_H;  // exposure time
        sensor_context->sensorRegs[0].astI2cData[3].u8DelayFrmNum = 0;
        sensor_context->sensorRegs[0].astI2cData[3].u32RegAddr = SC520SC_ANA_AGAIN;  // analog gain
        sensor_context->sensorRegs[0].astI2cData[4].u8DelayFrmNum = 0;
        sensor_context->sensorRegs[0].astI2cData[4].u32RegAddr = SC520SC_DIG_GAIN;  // analog gain
        sensor_context->sensorRegs[0].astI2cData[5].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[5].u32RegAddr = SC520SC_VTS_ADDR_L;  // VTS
        sensor_context->sensorRegs[0].astI2cData[6].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[6].u32RegAddr = SC520SC_VTS_ADDR_H;  // VTS
...
static int sc520sc_sensor_dump_info(void* snsHandle)
{
    int ret = 0;
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    sc520sc_read_register(snsHandle, SC520SC_VTS_ADDR_H, &reg_val_h);
    sc520sc_read_register(snsHandle, SC520SC_VTS_ADDR_L, &reg_val_l);
    vts = (reg_val_h << 8) | reg_val_l;
    sc520sc_read_register(snsHandle, SC520SC_EXPO_H, &reg_val_h);
    sc520sc_read_register(snsHandle, SC520SC_EXPO_L, &reg_val_l);
    exp_time = (reg_val_h << 8) | reg_val_l;
    sc520sc_read_register(snsHandle, SC520SC_ANA_AGAIN, &reg_val_h);
    again = reg_val_h;
    pthread_mutex_lock(&sensor_context->apiLock);
    CLOG_INFO("sc520sc regs(vts=%d,exptime=%d,again=0x%x,dain =0x%x),struct(initVTS=%d,initFps=%f,vts=%d,expline=%d)",
...
/*ae function*/
static int sc520sc_sensor_get_ae_default(void* snsHandle, uint32_t u32ChanelId, ISP_SENSOR_AE_DEFAULT_S* pstSensorAeDft)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    uint32_t exp_time = 0;
    uint32_t again = 0, dgain = 0;
    ...
    /* uint : us */
    // pstSensorAeDft->maxExpTime = (pstSensorState->initVTS - SC520SC_VTS_ADJUST) * sensor_context->lineTime / 1000;
    // pstSensorAeDft->minExpTime = SC520SC_EXPO_LINES_MIN * sensor_context->lineTime / 1000;

    pthread_mutex_unlock(&sensor_context->apiLock);
    return 0;
}

static int sc520sc_sensor_get_expotime_by_fps(void* snsHandle, float f32Fps)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    pthread_mutex_lock(&sensor_context->apiLock);
    minFps = (sensor_context->minVTS * sensor_context->maxFps) / SC520SC_VTS_LINES_MAX;
    ...
    max_expotime = (vts - SC520SC_VTS_ADJUST) * sensor_context->lineTime / 1000;  // us
out:
    pthread_mutex_unlock(&sensor_context->apiLock);
    return max_expotime;
}

static int sc520sc_sensor_fps_set(void* snsHandle, float f32Fps)
{
    int ret = 0;
    SENSOR_CONTEXT_S* sensor_context = NULL;
    uint32_t lines;
...
static int sc520sc_sensor_expotime_update(void* snsHandle, uint32_t u32ChanelId, uint32_t u32ExpoTime,
                                          ISP_SENSOR_VTS_INFO_S* pstSensorVtsInfo)
{
    ...
    expLine = (expLine < SC520SC_EXPO_LINES_MIN) ? SC520SC_EXPO_LINES_MIN : expLine;
    expLine = (expLine > (SC520SC_VTS_LINES_MAX - SC520SC_VTS_ADJUST)) ? (SC520SC_VTS_LINES_MAX - SC520SC_VTS_ADJUST)
                                                                       : expLine;
    sensor_context->hdrIntTime[u32ChanelId] = expLine * sensor_context->lineTime / 1000;

    // if (expLine > (sensor_context->initVTS - SC520SC_VTS_ADJUST))
    //     sensor_context->vts[0] = expLine + SC520SC_VTS_ADJUST;
    // else
    //     sensor_context->vts[0] = sensor_context->initVTS;
...
static int sc520sc_sensor_gain_update(void* snsHandle, uint32_t u32ChanelId, uint32_t* pAgainVal, uint32_t* pDgainVal)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    int ret = 0;
    uint32_t AGain_Reg, AGain_Reg_Fine = 0;
    uint32_t AGain_Val, DGain_Reg, step;
    uint16_t reg_gain;
	uint16_t temp_gain;
	uint16_t gain_index;
    ...
    pthread_mutex_lock(&sensor_context->apiLock);
    AGain_Val = *pAgainVal;  // Q12 -> Q10



	// if (AGain_Val <= 0x100) { /* 1 gain */
	// 	AGain_Reg = 0x00;
	// } else if (AGain_Val <= 0x200) { /* 2x gain */
	// 	AGain_Reg = 0x08;
	// } else if (AGain_Val <= 0x300) { /* 4xgain */
	// 	AGain_Reg = 0x09;
	// } else if (AGain_Val <= 0x400) { /* 8x gain */
	// 	AGain_Reg = 0x0B;
	// } else if (AGain_Val <= 0x500) { /* 16x gain */
    //     AGain_Reg = 0x0f;
	// }else { /* 32x gain */
	// 	AGain_Reg = 0x1f;
	// }

    uint16_t SC520CS_AGC_Param[SC520CS_SENSOR_GAIN_MAP_SIZE][2] = {
		{  1024,  0x00 },
		{  2048,  0x08 },
		{  4096,  0x09 },
		{  8192,  0x0B },
    ...
    // uint16_t SC520CS_AGC_Param2[SC520CS_SENSOR_DGAIN_MAP_SIZE][2] = {
	// 	{0x80, 1.000}, {0x84, 1.031}, {0x88, 1.063}, {0x8C, 1.094},
    //     {0x90, 1.125}, {0x94, 1.156}, {0x98, 1.188}, {0x9C, 1.219},
    //     {0xA0, 1.250}, {0xA4, 1.281}, {0xA8, 1.313}, {0xAC, 1.344},
    //     {0xB0, 1.375}, {0xB4, 1.406}, {0xB8, 1.438}, {0xBC, 1.469},
    //     {0xC0, 1.500}, {0xC4, 1.531}, {0xC8, 1.563}, {0xCC, 1.594},
    //     {0xD0, 1.625}, {0xD4, 1.656}, {0xD8, 1.688}, {0xDC, 1.719},
    //     {0xE0, 1.750}, {0xE4, 1.781}, {0xE8, 1.813}, {0xEC, 1.844},
    //     {0xF0, 1.875}, {0xF4, 1.906}, {0xF8, 1.938}, {0xFC, 1.969},
	// };

    reg_gain = gain2reg(AGain_Val);
/*  hs04 code for SR-AL6398A-01-12 by liluling at 2022/8/3 end */
	for (gain_index = SC520CS_SENSOR_GAIN_MAP_SIZE - 1; gain_index > 0; gain_index--)
		if (reg_gain >= SC520CS_AGC_Param[gain_index][0])
			break;
    AGain_Reg = SC520CS_AGC_Param[gain_index][1];
// //1024 * 1024 / 1024 = 1024
	temp_gain = reg_gain * SC520CS_SENSOR_GAIN_BASE / SC520CS_AGC_Param[gain_index][0];
    DGain_Reg = (temp_gain >> 3) & 0xff;


    sensor_context->sensorRegs[0].astI2cData[3].u32Data = AGain_Reg;     // bit[7:0] = Again[7:0]
    sensor_context->sensorRegs[0].astI2cData[4].u32Data = DGain_Reg;     // bit[7:0] = Again[7:0]

    *pAgainVal = AGain_Val;
    *pDgainVal = 4096;
    pthread_mutex_unlock(&sensor_context->apiLock);
    printf("pAgainVal: %d  AGain_Reg: %x, DGain_Reg: %x\n", *pAgainVal, AGain_Reg, DGain_Reg);

    return ret;
}

static int sc520sc_get_aelib_default_settings(void* snsHandle, uint32_t u32ChanelId,
...
/*awb function*/
static int sc520sc_sensor_get_awb_default(void* snsHandle, uint32_t u32ChanelId,
...
static int sc520sc_get_awblib_default_settings(void* snsHandle, uint32_t u32ChanelId,
...
static int sc520sc_power_on(SENSOR_CONTEXT_S* sensor_context)
{
...
/*******************************************************************/
static int sc520sc_init(void** pHandle, SENSOR_CUSTOM_S snr_custom)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    sensor_context->name = SC520SC_NAME;
    ...
    sc520sc_power_on(sensor_context);
...
static int sc520sc_power_off(SENSOR_CONTEXT_S* sensor_context)
{
    SENSORS_CHECK_PARA_POINTER(sensor_context);
...
static int sc520sc_deinit(void* handle)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    if (sensor_context->stream_on_flag == 1) {
        sc520sc_write_burst_register(handle, stream_off_regs, ARRAY_SIZE(stream_off_regs));
        sensor_context->stream_on_flag = 0;
    }

    // sensor_hw_reset(sensor_context->devId);
    sc520sc_power_off(sensor_context);
...
static int sc520sc_global_config(void* handle, SENSOR_WORK_INFO_S* work_info)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    int ret = 0;
    uint16_t reg_val = 0;
    ...
    ret = sc520sc_write_burst_register(handle, stream_soft_reset_regs, ARRAY_SIZE(stream_soft_reset_regs));
    if (ret) {
    ...
    ret = sc520sc_write_burst_register(handle, sensor_context->work_info.setting_table,
    ...
    /*
        if(work_info->test_pattern_mode == CC_SENSOR_TEST_PATTERN_COLOR_BARS){
            ret = sc520sc_write_burst_register(handle, color_bar_regs, ARRAY_SIZE(color_bar_regs));
        }
    */
    out:
        pthread_mutex_unlock(&sensor_context->apiLock);
...
static int sc520sc_set_param(void* handle, const SENSOR_INIT_ATTR_S* init_attr)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
...
static int sc520sc_stream_on(void* handle)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    int ret = 0;
    uint32_t i = 0;
    ...
    // for (i = 0; i < sensor_context->sensorRegs[0].u32RegNum; i++) {
    //     sc520sc_write_register(handle, sensor_context->sensorRegs[0].astI2cData[i].u32RegAddr,
    //                            sensor_context->sensorRegs[0].astI2cData[i].u32Data);
    // }

    ret = sc520sc_write_burst_register(handle, stream_on_regs, ARRAY_SIZE(stream_on_regs));
    usleep(10000);
...
static int sc520sc_stream_off(void* handle)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    int ret = 0;
    ...
    pthread_mutex_lock(&sensor_context->apiLock);
    ret = sc520sc_write_burst_register(handle, stream_off_regs, ARRAY_SIZE(stream_off_regs));
...
static int sc520sc_get_ops(void* handle, ISP_SENSOR_REGISTER_S* pSensorFuncOps)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    pSensorFuncOps->stSensorFunc.pfn_sensor_write_reg = sc520sc_sensor_write_reg;
    pSensorFuncOps->stSensorFunc.pfn_sensor_get_isp_default = sc520sc_sensor_get_isp_default;
    pSensorFuncOps->stSensorFunc.pfn_sensor_get_isp_black_level = sc520sc_sensor_get_isp_black_level;
    pSensorFuncOps->stSensorFunc.pfn_sensor_get_reg_info = sc520sc_sensor_get_reg_info;
    pSensorFuncOps->stSensorFunc.pfn_sensor_dump_info = sc520sc_sensor_dump_info;
    pSensorFuncOps->stSensorFunc.pfn_sensor_group_regs_start = sc520sc_sensor_group_reg_start;
    pSensorFuncOps->stSensorFunc.pfn_sensor_group_regs_done = sc520sc_sensor_group_reg_done;

    pSensorFuncOps->stSensorAeFunc.pfn_sensor_get_ae_default = sc520sc_sensor_get_ae_default;
    pSensorFuncOps->stSensorAeFunc.pfn_sensor_fps_set = sc520sc_sensor_fps_set;
    pSensorFuncOps->stSensorAeFunc.pfn_sensor_get_expotime_by_fps = sc520sc_sensor_get_expotime_by_fps;
    pSensorFuncOps->stSensorAeFunc.pfn_sensor_expotime_update = sc520sc_sensor_expotime_update;
    pSensorFuncOps->stSensorAeFunc.pfn_sensor_gain_update = sc520sc_sensor_gain_update;
    pSensorFuncOps->stSensorAeFunc.pfn_get_aelib_default_settings = sc520sc_get_aelib_default_settings;

    pSensorFuncOps->stSensorAwbFunc.pfn_sensor_get_awb_default = sc520sc_sensor_get_awb_default;
    pSensorFuncOps->stSensorAwbFunc.pfn_get_awblib_default_settings = sc520sc_get_awblib_default_settings;
    pthread_mutex_unlock(&sensor_context->apiLock);

    return 0;
}

static int sc520sc_detect_sensor(void* handle, SENSOR_VENDOR_ID_S* vendor_id)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    int ret = 0;
...
SENSOR_OBJ_S sc520csObj = {
    .name = SC520SC_NAME,
    .pfnInit = sc520sc_init,
    .pfnDeinit = sc520sc_deinit,
    .pfnGloablConfig = sc520sc_global_config,
    .pfnSetParam = sc520sc_set_param,
    .pfnStreamOn = sc520sc_stream_on,
    .pfnStreamOff = sc520sc_stream_off,
    .pfnGetSensorOps = sc520sc_get_ops,
    .pfnDetectSns = sc520sc_detect_sensor,
    .pfnWriteReg = sc520sc_write_register,
    .pfnReadReg = sc520sc_read_register,
};
