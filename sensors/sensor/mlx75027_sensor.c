...
#define SENSOR_MAGIC 0x1A2B3C // Define yourself
#define MLX75027_NAME "mlx75027"
static const unsigned int mlx75027_reg_addr_byte = I2C_16BIT; /*byte width of the sensor register address*/
static const unsigned int mlx75027_reg_data_byte = I2C_8BIT;  /*byte width of sensor register data*/
...
static struct regval_tab test_pattern_regs[] = {
    {0x1405, 0x00},
	{0x1406, 0x04},
	{0x1407, 0x01},// enable PN9 pattern
	// {0x1407, 0x00},// diasble PN9 pattern
};

#define MLX75027_VTS_ADJUST     (0) /* vts - max_exposure*/
#define MLX75027_VTS_LINES_MAX  (0xffff)
#define MLX75027_EXPO_LINES_MIN (0x0008)

#define MLX75027_VTS_ADDR_H (0x380E)
#define MLX75027_VTS_ADDR_L (0x380F)
#define MLX75027_EXPO_H     (0x3510)
#define MLX75027_EXPO_M     (0x3511)
#define MLX75027_EXPO_L     (0x3512)
#define MLX75027_AGAIN_H    (0x3508)
#define MLX75027_AGAIN_L    (0x3509)
#define MLX75027_DGAIN_H    (0x350A)
#define MLX75027_DGAIN_M    (0x350B)
#define MLX75027_DGAIN_L    (0x350C)
#define MLX75027_GROUP_ACCESS (0x3208)

/*******************************************************************/
static int mlx75027_write_register(void* handle, uint16_t regAddr, uint16_t value)
{
    ...
    reg_data.reg_len = mlx75027_reg_addr_byte;
    reg_data.val_len = mlx75027_reg_data_byte;
    reg_data.tab.reg = regAddr;
    reg_data.tab.val = value;
    //printf("%s-i2c_addr[0x%x]-Reg(0x%x)-Val(0x%x)\n", __FUNCTION__, reg_data.addr, reg_data.tab.reg, reg_data.tab.val);
    ret = sensor_write_register(sensor_context->devId, &reg_data);

    return ret;
}

static int mlx75027_read_register(void* handle, uint16_t regAddr, uint16_t* value)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    struct cam_i2c_data reg_data;
    int ret = 0;
...
static int mlx75027_write_burst_register(void* handle, struct regval_tab* reg_table, int reg_table_num)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    reg_table_data.reg_len = mlx75027_reg_addr_byte;
    reg_table_data.val_len = mlx75027_reg_data_byte;
...
#else
    {
        ...
        for (i = 0; i < reg_table_num; i++) {
            mlx75027_write_register(handle, reg_table[i].reg, reg_table[i].val);
        }
    }
#endif
    return ret;
...
#if 0
static int mlx75027_read_burst_register(void* handle, struct regval_tab* reg_table, int reg_table_num)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    struct cam_burst_i2c_data reg_table_data;
    int ret = 0;
...
/*******************************************************************/
/*isp sensor function*/
static int mlx75027_sensor_write_reg(void* snsHandle, uint32_t regAddr, uint32_t value)
{
    int ret = 0;
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    ret = mlx75027_write_register(snsHandle, regAddr, value);
    pthread_mutex_unlock(&sensor_context->apiLock);
    return ret;
}

static int mlx75027_sensor_group_reg_start(void* snsHandle)
{
    int ret = 0;
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    pthread_mutex_lock(&sensor_context->apiLock);
    mlx75027_write_register(snsHandle, MLX75027_GROUP_ACCESS, 0);
    pthread_mutex_unlock(&sensor_context->apiLock);
    return ret;
}

static int mlx75027_sensor_group_reg_done(void* snsHandle)
{
    int ret = 0;
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    pthread_mutex_lock(&sensor_context->apiLock);
    mlx75027_write_register(snsHandle, MLX75027_GROUP_ACCESS, 0x10);
    mlx75027_write_register(snsHandle, MLX75027_GROUP_ACCESS, 0xe0);
    pthread_mutex_unlock(&sensor_context->apiLock);
    return ret;
}

static int mlx75027_sensor_get_isp_default(void* snsHandle, uint32_t u32ChanelId, uint32_t camScene,
...
static int mlx75027_sensor_get_isp_black_level(void* snsHandle, uint32_t u32ChanelId,
...
static int mlx75027_sensor_get_reg_info(void* snsHandle, ISP_SENSOR_REGS_INFO_S* pstSensorRegsInfo)
{
// #if 0
    SENSOR_CONTEXT_S* sensor_context = NULL;
    uint32_t i = 0;
    ...
    if (false == sensor_context->syncInit) {
        ...
        for (i = 0; i < sensor_context->sensorRegs[0].u32RegNum; i++) {
            ...
            sensor_context->sensorRegs[0].astI2cData[i].u32AddrWidth = mlx75027_reg_addr_byte;
            sensor_context->sensorRegs[0].astI2cData[i].u32DataWidth = mlx75027_reg_data_byte;
        }


        sensor_context->sensorRegs[0].astI2cData[0].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[0].u32RegAddr = MLX75027_EXPO_L;  // exposure time
        sensor_context->sensorRegs[0].astI2cData[1].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[1].u32RegAddr = MLX75027_EXPO_M;  // exposure time
        sensor_context->sensorRegs[0].astI2cData[2].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[2].u32RegAddr = MLX75027_EXPO_H;  // exposure time
        sensor_context->sensorRegs[0].astI2cData[3].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[3].u32RegAddr = MLX75027_AGAIN_L;  // analog gain
        sensor_context->sensorRegs[0].astI2cData[4].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[4].u32RegAddr = MLX75027_AGAIN_H;  // analog gain
        sensor_context->sensorRegs[0].astI2cData[5].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[5].u32RegAddr = MLX75027_DGAIN_L;  // digital gain
        sensor_context->sensorRegs[0].astI2cData[6].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[6].u32RegAddr = MLX75027_DGAIN_M;  // digital gain
        sensor_context->sensorRegs[0].astI2cData[7].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[7].u32RegAddr = MLX75027_DGAIN_H;  // digital gain
        sensor_context->sensorRegs[0].astI2cData[8].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[8].u32RegAddr = MLX75027_VTS_ADDR_L;  // VTS
        sensor_context->sensorRegs[0].astI2cData[9].u8DelayFrmNum = 2;
        sensor_context->sensorRegs[0].astI2cData[9].u32RegAddr = MLX75027_VTS_ADDR_H;  // VTS
...
static int mlx75027_sensor_dump_info(void* snsHandle)
{
    int ret = 0;
#if 0
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    mlx75027_read_register(snsHandle, MLX75027_VTS_ADDR_H, &reg_val_h);
    mlx75027_read_register(snsHandle, MLX75027_VTS_ADDR_L, &reg_val_l);
    vts = ((reg_val_h & 0x7f) << 8) | reg_val_l;
    mlx75027_read_register(snsHandle, MLX75027_EXPO_H, &reg_val_h);
    mlx75027_read_register(snsHandle, MLX75027_EXPO_M, &reg_val_m);
    mlx75027_read_register(snsHandle, MLX75027_EXPO_L, &reg_val_l);
    exp_time = (reg_val_h << 16) | (reg_val_m << 8) | reg_val_l;
    mlx75027_read_register(snsHandle, MLX75027_AGAIN_H, &reg_val_h);
    mlx75027_read_register(snsHandle, MLX75027_AGAIN_L, &reg_val_l);
    again = ((reg_val_h & 0xf) << 7) | ((reg_val_l & 0x7e) >> 1);
    mlx75027_read_register(snsHandle, MLX75027_DGAIN_H, &reg_val_h);
    mlx75027_read_register(snsHandle, MLX75027_DGAIN_M, &reg_val_m);
    mlx75027_read_register(snsHandle, MLX75027_DGAIN_L, &reg_val_l);
    dgain = ((reg_val_h & 0x3) << 10) | (reg_val_m << 2) | ((reg_val_l & 0xc0) >> 6);

    pthread_mutex_lock(&sensor_context->apiLock);
    CLOG_INFO("mlx75027 regs(vts=%d,exptime=%d,again=0x%x,dain =0x%x),struct(initVTS=%d,initFps=%f,vts=%d,expline=%d)",
...
/*ae function*/
static int mlx75027_sensor_get_ae_default(void* snsHandle, uint32_t u32ChanelId, ISP_SENSOR_AE_DEFAULT_S* pstSensorAeDft)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    uint32_t exp_time = 0;
    uint32_t again = 0, dgain = 0;
    ...
    /* uint : us */
    // pstSensorAeDft->maxExpTime = (pstSensorState->initVTS - MLX75027_VTS_ADJUST) * sensor_context->lineTime / 1000;
    // pstSensorAeDft->minExpTime = MLX75027_EXPO_LINES_MIN * sensor_context->lineTime / 1000;

    pthread_mutex_unlock(&sensor_context->apiLock);
    return 0;
}

static int mlx75027_sensor_get_expotime_by_fps(void* snsHandle, float f32Fps)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    pthread_mutex_lock(&sensor_context->apiLock);
    minFps = (sensor_context->minVTS * sensor_context->maxFps) / MLX75027_VTS_LINES_MAX;
    ...
    max_expotime = (vts - MLX75027_VTS_ADJUST) * sensor_context->lineTime / 1000;  // us
out:
    pthread_mutex_unlock(&sensor_context->apiLock);
    return max_expotime;
}

static int mlx75027_sensor_fps_set(void* snsHandle, float f32Fps)
{
    int ret = 0;
    SENSOR_CONTEXT_S* sensor_context = NULL;
    uint32_t lines;
    ...
    if (expLine <= (sensor_context->initVTS - MLX75027_VTS_ADJUST)) {
...
static int mlx75027_sensor_expotime_update(void* snsHandle, uint32_t u32ChanelId, uint32_t u32ExpoTime,
                                          ISP_SENSOR_VTS_INFO_S* pstSensorVtsInfo)
{
    ...
    expLine = (expLine < MLX75027_EXPO_LINES_MIN) ? MLX75027_EXPO_LINES_MIN : expLine;
    expLine = (expLine > (MLX75027_VTS_LINES_MAX - MLX75027_VTS_ADJUST)) ? (MLX75027_VTS_LINES_MAX - MLX75027_VTS_ADJUST)
                                                                       : expLine;
    sensor_context->hdrIntTime[u32ChanelId] = expLine * sensor_context->lineTime / 1000;

    if (expLine > (sensor_context->initVTS - MLX75027_VTS_ADJUST))
        sensor_context->vts[0] = expLine + MLX75027_VTS_ADJUST;
    else
        sensor_context->vts[0] = sensor_context->initVTS;
...
static int mlx75027_sensor_gain_update(void* snsHandle, uint32_t u32ChanelId, uint32_t* pAgainVal, uint32_t* pDgainVal)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    int ret = 0;
...
static int mlx75027_get_aelib_default_settings(void* snsHandle, uint32_t u32ChanelId,
...
/*awb function*/
static int mlx75027_sensor_get_awb_default(void* snsHandle, uint32_t u32ChanelId,
...
static int mlx75027_get_awblib_default_settings(void* snsHandle, uint32_t u32ChanelId,
...
static int mlx75027_power_on(SENSOR_CONTEXT_S *sensor_context)
{
    SENSORS_CHECK_PARA_POINTER(sensor_context);
    CLOG_INFO("%s: power on!", __FUNCTION__);
    /*  from \kernel\linux-4.9\drivers\media\platform\sunxi-vin\modules\sensor\mlx75027_mipi.c
		sensor_print("PWR_ON!\n");
		vin_gpio_set_status(sd, PWDN, 1);
		vin_gpio_set_status(sd, RESET, 1);
        vin_set_mclk(sd, ON);
		usleep_range(10000, 12000); // 1

        vin_gpio_write(sd, RESET, CSI_GPIO_LOW);
		vin_gpio_write(sd, PWDN, CSI_GPIO_LOW);
        usleep_range(10000, 12000); // 2

		vin_set_pmu_channel(sd, IOVDD, ON);
		usleep_range(10000, 12000); // 3

		vin_set_pmu_channel(sd, DVDD, ON);
		usleep_range(10000, 12000); // 4

		vin_set_pmu_channel(sd, AVDD, ON);
		usleep_range(20000, 22000); // 5

        vin_set_pmu_channel(sd, CAMERAVDD, ON);
        usleep_range(10000, 20000); // 6

        vin_gpio_write(sd, RESET, CSI_GPIO_HIGH);
		vin_gpio_write(sd, PWDN, CSI_GPIO_HIGH);
        usleep_range(10000, 20000); // 7
     */
...
static int mlx75027_power_off(SENSOR_CONTEXT_S *sensor_context)
{
    /* 	from \kernel\linux-4.9\drivers\media\platform\sunxi-vin\modules\sensor\mlx75027_mipi.c
...
static int mlx75027_init(void** pHandle, SENSOR_CUSTOM_S snr_custom)
{
    ...
    sensor_context->name = MLX75027_NAME;
    ...
    mlx75027_power_on(sensor_context);
...
static int mlx75027_deinit(void* handle)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    if (sensor_context->stream_on_flag == 1) {
        mlx75027_write_burst_register(handle, stream_off_regs, ARRAY_SIZE(stream_off_regs));
        sensor_context->stream_on_flag = 0;
    }

    // sensor_hw_reset(sensor_context->devId);
    mlx75027_power_off(sensor_context);
...
static int mlx75027_global_config(void* handle, SENSOR_WORK_INFO_S* work_info)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    int ret = 0;
    ...
    // ret = mlx75027_write_burst_register(handle, stream_soft_reset_regs, ARRAY_SIZE(stream_soft_reset_regs));
    // if (ret) {
    //     goto out;
    // }
    usleep(5000);
    ret = mlx75027_write_burst_register(handle, sensor_context->work_info.setting_table,
    ...
    if(work_info->test_pattern_mode == CC_SENSOR_TEST_PATTERN_COLOR_BARS){
        // ret = mlx75027_write_burst_register(handle, color_bar_regs, ARRAY_SIZE(color_bar_regs));
    }
    else if(work_info->test_pattern_mode == CC_SENSOR_TEST_PATTERN_PN9)
    {
        ret = mlx75027_write_burst_register(handle, test_pattern_regs, ARRAY_SIZE(test_pattern_regs));
    }
...
static int mlx75027_set_param(void* handle, const SENSOR_INIT_ATTR_S* init_attr)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
...
static int mlx75027_stream_on(void* handle)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    int ret = 0;
    uint32_t i = 0;
    ...
    if (ret)
        goto out;
    for (i = 0; i < sensor_context->sensorRegs[0].u32RegNum; i++) {
        mlx75027_write_register(handle, sensor_context->sensorRegs[0].astI2cData[i].u32RegAddr,
                               sensor_context->sensorRegs[0].astI2cData[i].u32Data);
    }
    ret = mlx75027_write_burst_register(handle, stream_on_regs, ARRAY_SIZE(stream_on_regs));
...
static int mlx75027_stream_off(void* handle)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    int ret = 0;
    ...
    pthread_mutex_lock(&sensor_context->apiLock);
    ret = mlx75027_write_burst_register(handle, stream_off_regs, ARRAY_SIZE(stream_off_regs));
...
static int mlx75027_get_ops(void* handle, ISP_SENSOR_REGISTER_S* pSensorFuncOps)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    ...
    pSensorFuncOps->stSensorFunc.pfn_sensor_write_reg = mlx75027_sensor_write_reg;
    pSensorFuncOps->stSensorFunc.pfn_sensor_get_isp_default = mlx75027_sensor_get_isp_default;
    pSensorFuncOps->stSensorFunc.pfn_sensor_get_isp_black_level = mlx75027_sensor_get_isp_black_level;
    pSensorFuncOps->stSensorFunc.pfn_sensor_get_reg_info = mlx75027_sensor_get_reg_info;
    pSensorFuncOps->stSensorFunc.pfn_sensor_dump_info = mlx75027_sensor_dump_info;
    pSensorFuncOps->stSensorFunc.pfn_sensor_group_regs_start = mlx75027_sensor_group_reg_start;
    pSensorFuncOps->stSensorFunc.pfn_sensor_group_regs_done = mlx75027_sensor_group_reg_done;

    pSensorFuncOps->stSensorAeFunc.pfn_sensor_get_ae_default = mlx75027_sensor_get_ae_default;
    pSensorFuncOps->stSensorAeFunc.pfn_sensor_fps_set = mlx75027_sensor_fps_set;
    pSensorFuncOps->stSensorAeFunc.pfn_sensor_get_expotime_by_fps = mlx75027_sensor_get_expotime_by_fps;
    pSensorFuncOps->stSensorAeFunc.pfn_sensor_expotime_update = mlx75027_sensor_expotime_update;
    pSensorFuncOps->stSensorAeFunc.pfn_sensor_gain_update = mlx75027_sensor_gain_update;
    pSensorFuncOps->stSensorAeFunc.pfn_get_aelib_default_settings = mlx75027_get_aelib_default_settings;

    pSensorFuncOps->stSensorAwbFunc.pfn_sensor_get_awb_default = mlx75027_sensor_get_awb_default;
    pSensorFuncOps->stSensorAwbFunc.pfn_get_awblib_default_settings = mlx75027_get_awblib_default_settings;
    pthread_mutex_unlock(&sensor_context->apiLock);

    return 0;
}

static int mlx75027_detect_sensor(void* handle, SENSOR_VENDOR_ID_S* vendor_id)
{
    SENSOR_CONTEXT_S* sensor_context = NULL;
    int ret = 0;
...
SENSOR_OBJ_S mlx75027Obj = {
    .name = MLX75027_NAME,
    .pfnInit = mlx75027_init,
    .pfnDeinit = mlx75027_deinit,
    .pfnGloablConfig = mlx75027_global_config,
    .pfnSetParam = mlx75027_set_param,
    .pfnStreamOn = mlx75027_stream_on,
    .pfnStreamOff = mlx75027_stream_off,
    .pfnGetSensorOps = mlx75027_get_ops,
    .pfnDetectSns = mlx75027_detect_sensor,
    .pfnWriteReg = mlx75027_write_register,
    .pfnReadReg = mlx75027_read_register,
};
