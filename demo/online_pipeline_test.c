...
int fillVrfInfo(VRF_INFO_S *mVrf, FRAME_INFO_S* frameInfo, int width, int height, int depth)
{
    if (!mVrf || !frameInfo)
        return -1;
...
int32_t ispStartDumpRaw(TUNING_MODULE_TYPE_E type, uint32_t groupId, uint32_t count,
...
int32_t ispEndDumpRaw(TUNING_MODULE_TYPE_E type, uint32_t groupId)
{
    asrTuning_BUFFER_INFO_S *tunBuffer = NULL;
    asrISP_BUFFER_INFO_S *isp_buffer_info = NULL;
    uint32_t pipelineID;
